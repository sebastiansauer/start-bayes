...
Außerdem ist die Regressionsanalyse (für viele Situationen) die beste Heransgehensweise, 
da sie viele Möglichkeiten für Erweiterungen bietet.
Entsperchend ist das Thema dieses Kapitels gängige Forschungsfragen 
mit Hilfe Regressionsanalyse zu untersuchen.
...
abgesehen von einer paar Erweiterungen.
...
Das sollten wir in einem nächsten Mal verbessern.
...
von Kindern mit Mütter mit Schlulabschluss (im Vergleich zum IQ-Wert 
von Kindern mit Mütter ohne Schlusabschluss). 
...
In der *frequentistischen* Statistik (die mehrheitlich unterricht wird) untersucht man diese Datensituation -- Mittelwertsdifferenz zwischen zwei Gruppen -- mit einem sog. *t-Test*.
...
wie wahrscheinlich es auf Basis des Modell ist, 
noch extremere Ergebnisse zu beachten unter der Annahme, dass die (exakte) Nullhypothese wahr ist. 
...
das in etwa gleiche Ergebnis erreichen.^[Genauer gesagt, erlaubt der t-Test in der Form des Welche-Tests auch Abweichungen von der Varianzhomogenität der gestesten Gruppen. Die Regression geht hingegen von Varianzhomogenität (Homoskedastizität) aus. 
...
In der Regressionsformel sieht man, dass ein zusätzlicher Parametern, eben der Interaktionseffekt, 
...
Der *Achsenabschnitt* gibt die IQ-Schätzwerte für Kinder mit Mütter ohne Abschluss und mit einem IQ von 0 an. 
...
*Interaktion* (`mom_hs:mom_iq`): Der Unterschied in den Steigungen der Regressiongeraden, also der Unterschied 
des Koeffizienten für `mom_iq` zwischen Mütter mit bzw. ohne Schulabschluss.
...
Wo man man über mehr inhaltliches Wissen verfügt, so wird man die Prioris anpassen wollen.
...
Der Begriff "Zusammenhang" ist nicht exakt.
 Häufig wird er (für metrische Variablen) verstanden als lineare Korrelation $\rho$ bzw. $r$  lineare Regression $\beta$, bzw. $b$. 
...
Berechnen wir im Foglenden das multiple Modell (beide Prädiktoren), `m10.9`.
...
Das bedeutet anschaulich, man betrachtet den den Zusammenhang einee UV mit der AV, wobei man gleichzeitig den anderen Prädiktor konstant hält.
...
In diese Abschnitt berechnen wir ein Modell (Modell `m10.12`), in dem sowohl die Prädiktoren z-transformiert sind (standardisiert) *als auch* die *AV*.
...
um `kid_score` vorher zusagen, findet man einen starken Zusammenhang.
...
Was ist eigentlich der numerische Unterschied Bayesianischen vs. Frequentistischen Modellen?
...
:::callout-important
Wenn auch die  Ergebnisse eines Frequentistischen und Bayes-Modell numerisch ähnlich sein können, 
