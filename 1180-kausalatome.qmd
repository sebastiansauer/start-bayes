...
library(ggrepel)  # Labels direkt an Linien/Punkte setzen
```



Wir nutzen wieder den Datensatz [Saratoga County](https://vincentarelbundock.github.io/Rdatasets/csv/mosaicData/SaratogaHouses.csv)^[<https://vincentarelbundock.github.io/Rdatasets/csv/mosaicData/SaratogaHouses.csv>]; s. @tbl-saratoga.
Hier gibt es eine 
[Beschreibung des Datensatzes](https://vincentarelbundock.github.io/Rdatasets/doc/mosaicData/SaratogaHouses.html)^[<https://vincentarelbundock.github.io/Rdatasets/doc/mosaicData/SaratogaHouses.html>].
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/1180-kausalatome.html).
:::
:::
...
[Gott ist gerecht (?)](https://twitter.com/TheTweetOfGod/status/1462594155176026123)^[<https://twitter.com/TheTweetOfGod/status/1462594155176026123>]
...
#| echo: false
#| eval: false
ggdag_adjust(coll1_dag, var = "date", text_size = 4.5) +
  theme_dag()
```
...
p_coll_dag2 <- ggdag(coll2_dag, use_labels = "label", text_size = 4.5)  + theme_dag_blank()
p_coll_dag2
...
-- oft vorschnell -- als kausal. 
(Ehrlicherweise muss man zugeben, dass auch wissenschaftliche Berichte Daten über Zusammenhänge gerne kausal interpretieren, oft vorschnell.)
...
d_eignung2 <-
  d_eignung |>
  mutate(studium = factor(studium, labels = c("kein Studium", "Studium")))

# Endpunkte der Regressionsgeraden je Gruppe, um die Labels dort zu platzieren
label_data <-
  d_eignung2 |>
  group_by(studium) |>
  group_modify(~ {
    fleiss_max <- max(.x$fleiss)
    tibble(fleiss = fleiss_max,
           iq = predict(lm(iq ~ fleiss, data = .x),
                        newdata = tibble(fleiss = fleiss_max)))
  }) |>
  ungroup()
...
Ein Hintertürpfad entsteht, wenn es eine alternative Route über eine oder mehrere Variable gibt, die UV mit der AV verbindet. 
Dieser Pfad verzerrt die Schätzwerte des kausalen Einflusses, wenn er nicht kontrolliert wird. $\square$
:::
...
Wenn wir den Häusern zufällig (randomisiert) eine Anzahl von Schlafzimmern (`b`) zuweisen könnten (unabhängig von ihrer Quadratmeterzahl, `a`), 
würde sich der Graph so ändern.
...
Sollte man den Mediator `m` in @fig-med1 kontrollieren, wenn man den Kausaleffekt von `x` auf `y` schätzen möchte?^[Nein, durch das Kontrollieren von `m` wird der Kausalpfad von `x` zu `y` geschlossen. 
Es kann keine kausale Assoziation von `x` auf `y` mehr "fließen".] $\square$
:::
...
Alle Pfade, die von der UV (`X`) *wegführen*, sind entweder "gute" Kausalpfade oder automatisch geblockte Nicht-Kausal-Pfade. 
In diesem Fall müssen wir nichts tun.
Denken Sie daran, dass Sie keine Nachkommen der UV kontrollieren dürfen, da das den Kausalpfad von der UV zur AV blockieren könnte.

#### Fall 2: `->x`
$\boxed{\rightarrow X}$

Alle Pfade, die zur UV *hinführen*, sind immer Nicht-Kausal-Pfade, *Hintertüren*. 
Diese Pfade können offen sein, dann müssen wir sie schließen. Sie können auch geschlossen sein, dann müssen wir nichts tun.
...
[Mehr Infos](https://bookdown.org/content/4857/the-haunted-dag-the-causal-terror.html#backdoor-waffles)^[<https://bookdown.org/content/4857/the-haunted-dag-the-causal-terror.html#backdoor-waffles>]
...
Bei Plus und Minus soll man eine Bewertung von Positiven bzw. Negativen bzgl. eines Sachverhaltes anführen. 
Bei *Interessant* verzichtet man aber explizit auf eine Bewertung (im Sinne von "gut" oder "schlecht") 
und fokussiert sich auf Interessantes, Überraschendes, Bemerkenswertes.
...
Ein gutes Lehrbuch, das auf Kausalinferenz eingeht, ist @huntington-klein2022. Praktischerweise ist es [öffentlich lesbar](https://theeffectbook.net/ch-CausalPaths.html)^[<https://theeffectbook.net/ch-CausalPaths.html>].
Das Web-Buch [Causal Inference for the Brave and True](https://matheusfacure.github.io/python-causality-handbook/landing-page.html)^[<https://matheusfacure.github.io/python-causality-handbook/landing-page.html>] sieht auch vielversprechend aus.
Es gibt viel Literatur zu dem Thema; relevante Suchterme sind z. B. "DAG", "causal" oder "causal inference".
...
1. [kollision-eignung](https://sebastiansauer.github.io/datenwerk/posts/kollision-eignung/kollision-eignung.html)
2. [hintertuer](https://sebastiansauer.github.io/datenwerk/posts/hintertuer/hintertuer.html)
3. [tutorium-kausal](https://sebastiansauer.github.io/datenwerk/posts/tutorium-kausal/index.html)
