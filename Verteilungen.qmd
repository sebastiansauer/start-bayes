...
Abb. @fig-mtcars-freq visualisiert die Häufigkeitsverteilung von `cyl`.

```{r}
#| echo: false
#| fig-cap: Häufigkeitsverteilung von `cyl`
...
Ein *stetiges* Merkmal, wie `hp` (PS-Zahl), lässt sich durch Klassenbildung diskretisieren, s. Abb. @fig-mtcars-hp.
    
```{r Normalverteilung-2}
#| fig-cap: Häufigkeitsverteilng einer stetigen Variablen, diskretisiert in 10 "Körbe" oder Gruppen
#| label: fig-mtcars-hp
#| echo: false
mtcars %>% 
  ggplot(aes(x = hp)) +
...
Warhscheinlichkeitsverteilungen dienen dazu, Ereignissen einer Zufallsvariable eine Wahrscheinlichkeit zuzuordnen.
...
Stellen wir uns eine Kistchen^[In den Lehrbüchern häufig als Urne bezeichnet, was den bösen Spott von "Friedhofsstatistik" nach sich zog.] mit 5 Losen vor, darunter 2 *T*reffer (Gewinn) und 3 *N*ieten, s. Abb. @fig-urne.
...
Wie groß ist die Wahrscheinlichkeit von $A^{\prime}$, d.h. bei $n=4$ Zügen $x=2$ Treffer zu erzielen?
...
Die Verteilung der Zufallsvariablen IQ ist normalverteilt mit einem Mittelwert von 100 und einer Streuung von 15:
...
Ziehen wir zufällig $1e4$ Stichproben aus $\mathcal{N}(100,15)$ und berechen die Quantile:
...
- Die *Verteilungsfunktion F* für einen Wert $x$ gibt die Wahrscheinlichkeit an, dass die zugehörige Zufallsvariable $X$ höchstens so groß wie $x$ annimt Sie zeigt also die kumulierte Wahrscheinlichkeit $[-\infty, q)$.
...
Wir gehen von einer Normalverteilung aus, wie sie zur Beschreibung von Intelligenz (IQ) verwendet wird.
