...
//| echo: false
textinfo = md`
Von insgesamt ${nrep} Stichproben, ${d3.sum(data.map(d => d.rep <= nrep && (mu >= d.lower && mu <= d.upper)))} (**<span style="color:red">${(d3.sum(data.map(d => d.rep <= nrep && (mu >= d.lower && mu <= d.upper)))*100/nrep).toFixed(3)}%</span>**) von den ${confidence}% Konfindenzintervallen enthielten den wahren (tatsächlichen) Populationswert ${tex`\mu= `} ${mu}.
`
```
