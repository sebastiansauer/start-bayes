...
- Spellcheck tracking: each chapter file has an `<!-- spellcheck: YYYY-MM-DD -->` marker comment right after its top-level heading (empty date until first checked). After proofreading a chapter, fill in that chapter's marker with the current date, then run `Rscript get-spellcheck-status.R` to regenerate `spellcheck-status.qmd` (an auto-generated status table — don't hand-edit it).
