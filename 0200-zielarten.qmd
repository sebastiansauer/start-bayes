...
- [Statistik 1, Abschnitt zur Normalverteilung](https://sebastiansauer.github.io/statistik1/040-verbildlichen#spezialfall-normalverteilung)^[<https://sebastiansauer.github.io/statistik1/040-verbildlichen#spezialfall-normalverteilung>]
...
::: {.content-visible when-format="html"}
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/0200-zielarten.html).
:::
:::
...
Wenn das Ziel die Beschreibung eines Datensatzes ist,
so spricht man von *deskriptiver* Statistik.



Statistische Analysen mit dem Ziel zu beschreiben, fassen die Daten zusammen (zu möglichst aussagekräftigen Kennzahlen).
Beschreibende Statistik nennt man auch *deskriptive Statistik*; 
Mittelwert, Korrelation und Regressionskoeffizienten sind typische Kennzahlen ("Statistiken");  s. @fig-beschreiben.
...
![Die deskriptive Statistik fasst eine Spalte zu einer einzelnen Zahl zusammen.](img/desk-stats-zsmnfassen-crop.svg){#fig-desk width=50%}


![Zwei Spalten werden zu einer Zahl zusammengefasst](img/desk-stats-zsmnfassen-cor-crop.svg){#fig-desk2 width=50%}
...
::: {#exm-desk}
In einem Hörsaal sitzen 100 Studis. Alle schreiben ihre Körpergröße auf einen Zettel.
Die Dozentin sammelt die Zettel ein und rechnet dann den Mittelwert der Körpergröße der anwesenden Studierenden aus. Voilà: Deskriptive Statistik! $\square$
:::
...
Wenn das Ziel die Vorhersage bestimmter Ergebnisse ist,
spricht man von *prädiktiven* bzw. *prognostischen* Analysen.
...
- Wie schwer ist wohl Herr X? Er ist ein deutscher erwachsener Mann der Größe 1,80 m; mehr wissen wir nicht.
...
Wir können vom Modell eine einzelne Zahl (83) als Punkt-Schätzwert bzw. Vorhersagewert erhalten oder einen Schätzbereich (62-104 Punkte). $\square$
:::

Hier ist das Regressionsmodell für Ali (`lm_ali`), s. @lst-ali.

```{r}
#| lst-label: lst-ali
#| lst-cap: Regressionsmodell für Klausurerfolg als Funktion der Lernzeit (`lm_ali`)
noten2 <- read.csv("data/noten2.csv")
lm_ali <- lm(y ~ x, data = noten2)
```
...
lm1_b0 <- coef(lm_ali)[1]
lm1_b2 <- coef(lm_ali)[2]

ali_punkte <- predict(lm_ali, newdata = data.frame(x=42))
ali_vorhersageintervall <- predict(lm_ali, newdata =  data.frame(x=42),
...
p_ali_punktschaetzung <- 
  ...
  annotate("point", x = 42, y = ali_punkte, color = ycol,
           alpha = .7, size = 3) +
  scale_x_continuous(breaks = c(20, 40, 60, 80, 100)) +
  labs(x = "Lernzeit",
       y = "Klausurpunkte") +
  theme_large_text() +
  geom_label_repel(data = data.frame(x = 42, y = ali_punkte), label = "Ali",
       force = 20,
...
```{r}
#| label: fig-studie-a
#| fig-cap: "Überlebensrate je Geschlecht, mit bzw. ohne Medikament"
#| echo: false
...
Das Geschlecht (Östrogen) hat einen positiven (+) Einfluss auf die Einnahme des Medikaments und negativen Einfluss (-) auf Heilung.
...
den Gesamtdatensatz in Gruppen oder "Schichten" ("Strata") aufzuteilen.
...
ist nur möglich, da das Kausalmodell bekannt ist.
...
Die zweite Bedeutung von "Erklären" ist die **Populationsinferenz**:
Von einer *Stichprobe* auf die *Grundgesamtheit* (Population) schließen.

Anders als bei der Kausalinferenz geht es hier nicht um die Frage,
*welche* Variable Ursache und welche Wirkung ist,
sondern darum, ob ein in der Stichprobe beobachteter Zusammenhang (oder Unterschied)
auch in der Grundgesamtheit gilt -- oder ob er nur zufällig in dieser Stichprobe aufgetreten ist.
...
Populationsinferenz ist Thema des nächsten Kapitels (@sec-inferenz).
...
Die Statistik kann man verstehen als ein Verfahren, das wissenschaftliche Modelle in statistische übersetzt
und letztere dann einer empirischen Analyse unterzieht.
Alle statistischen Ergebnisse beruhen auf Modellen und sind nur insoweit gültig, wie das zugrundeliegende Modell gültig ist.
...
Interaktives Beispiel für ein lineares Modell. Fügen Sie Punkte per Klick/Touch hinzu.

::::
:::::


Probieren Sie [diese App](https://gallery.shinyapps.io/simple_regression/) aus, um
Regressionskoeffizienten, Steigung (slope) und Achsenabschnitt (Intercept), optimal in den Punkteschwarm zu legen.
Dabei meint "optimal", die Abweichungen (Residuen, Residualfehler; die roten Balken in der App) zu minimieren.^[<https://gallery.shinyapps.io/simple_regression/>]
Probieren Sie [diese App](https://shinyapps.org/showapp.php?app=https://shiny.psy.lmu.de/felix/lmfit&by=Felix%20Sch%C3%B6nbrodt&title=Find-a-fit!&shorttitle=Find-a-fit!) aus, die Ihnen gestattet, selber Hand an eine Regressionsgerade zu legen.^[<https://shinyapps.org/showapp.php?app=https://shiny.psy.lmu.de/felix/lmfit&by=Felix%20Sch%C3%B6nbrodt&title=Find-a-fit!&shorttitle=Find-a-fit!>]
...
Lesen Sie [diesen Post](https://setosa.io/ev/ordinary-least-squares-regression/)^[<https://setosa.io/ev/ordinary-least-squares-regression/>], 
...
Alternativ zur Regression könnte man sich in den Wald der statistischen Verfahren begeben, [wie hier von der Uni Münster als Ausschnitt (!) aufgeführt](https://web.archive.org/web/20091029162244/http://www.wiwi.uni-muenster.de/ioeb/en/organisation/pfaff/stat_overview_table.html)^[<https://web.archive.org/web/20091029162244/http://www.wiwi.uni-muenster.de/ioeb/en/organisation/pfaff/stat_overview_table.html>].
...
Wähle die Regression. Oder die Wahl der Verfahren. Spoiler: Nimm lieber die Regression.
:::
...
1. [ziele-statistik](https://sebastiansauer.github.io/datenwerk/posts/ziele-statistik/ziele-statistik.html)
...
quiz_inferenz_files <- list(
  ...
  "exr/zielart-beschreiben-definition-schoice/zielart-beschreiben-definition-schoice.Rmd",
  "exr/zielart-vorhersagen-punkt-bereich-schoice/zielart-vorhersagen-punkt-bereich-schoice.Rmd",
