...
Wir benötigen den Datensatz *!Kung*. 
...
In diesem Kapitel befragen wir die Post-Verteilung für ein normalverteilte Zufallsvariable, 
nämlich die Körpergröße der !Kung San. 
...
`{easystats`} macht das tatsächlich recht easy, s. @tbl-kung.
...
Wir sind uns über diesen Mittelwert nicht sicher^[Darum machen wir hier ja die ganz Show!], 
...
5. Je größer die Rate $\lambda$, desto *kleiner* die Streuung und der Mittelwert der Verteilung (und umgekehrt: Je größer  $1/\lambda$, desto *größer* die Streuung und der Mittelwert der Verteilung.)
...
für den man das Quantil haben möchte. (;it `pexp()` kann man sich analaog die die Verteilungsfunktion ausgeben lassen.) Mit `rate` wird $\lambda$ (lambda) bezeichnet.^[Es gibt auch [Online-Apps, die diese Werte ausgeben](https://homepage.divms.uiowa.edu/~mbognar/applets/exp-like.html.]
...
#| fig-cap: Unser (erstes) Kung-Modell (m41)
#| fig-asp: 0.4
plots(p1, p2)
```
...
::: {.column width="40%"}
![Modellschema für das Modell `m41`](img/modell-kung1.png){#fig-mod-kung1}
:::
...
Der Golem hört auf den Namen `rstanarm`^[Hey, *ich* habe ihn diesen Namen nicht gegeben.].
...
Nennen wir das Modell `m41`^[*m* wie Modell und 4, weil das Modell in Kapitel 4 von @mcelreath2020 in ähnlicher Form berichtet wird, und 1 weil es unsere erste Variante dieses Modells ist.].
...
`stan_glm`^[aus dem R-Paket `rstanam`, das zuvor installiert und gestartet sein muss, bevor Sie den Befehl nutzen können] ist eine Funktion, mit der man Regressionsmodelle berechnen kann.
...
Wir greifen damit auf die *Voreinstellung* (defaults) der Prioris von `rstanarm` zurück.
...
#| fig-cap: "Die gemeinsame Post-Verteilung von Mittelwert und Streuung von m42"
...
Natürlich können wir auch nur von einem einzelnen Parameter (z.B.  Mittelwert) die Verteilung untersuchen, s. @fig-m42-post-anders2.

```{r}
#| echo: false
#| fig-cap: Die Post-Verteilung von mu in m42; ein Balkendiagramm bietet sich an.
#| label: fig-m42-post-anders2
p20
```
...
Mit dem Befehl `parameters` kann man sich die geschätzten Parameterwerte einfach ausgeben lassen (s. @fig-m42-post-joint).
...
ci_low <- parameters(m41)$CI_low |> round(2)
ci_high <- parameters(m41)$CI_high |> round(2)
```


Das Wesentliche: Unser Golem schätzt den Größenmittelwert der Kung auf ca. 155cm bzw. auf einen Bereich von etwa `r ci_low` bis `r ci_high` schätzt.
...
Wer Näheres wissen will, findet [hier](https://easystats.github.io/easystats/) einen Anfang. Außerdem sei an @mcelreath2020 und @gelman2021 verwiesen. $\square$
:::
...
Außerdem wählen wir einen einen Likelihood-Funktion, so dass wir die Likelihood berechnen können.
...
Mittelwerte und Streuungen der AV werden als Grundlage für die Priori-Verteilungen herangezogen.
...
Häufig sind mehrere Entscheidungen möglich.
...
also nur wenig Priori-Information  in das Modell einfließen lässt.
:::
...
Außerdem ist der Name der ersten Spalte eigentlich unzulässig, da Spaltennamen in R nicht mit Sonderzeichen anfangen dürfen (sondern mit Buchstaben). Daher müssen wir den Namen mit "Samthandschuhen" anpacken.
...
1. [Priorwahl1](https://sebastiansauer.github.io/Datenwerk/posts/Priorwahl1/Priorwahl1.html)
2. [ReThink4e2](https://sebastiansauer.github.io/Datenwerk/posts/ReThink4e2/ReThink4e2.html)
3. [ReThink4e3](https://sebastiansauer.github.io/Datenwerk/posts/ReThink4e3/ReThink4e3.html)
4. [groesse03](https://sebastiansauer.github.io/Datenwerk/posts/groesse03/)
...
1. [stan_glm01](https://sebastiansauer.github.io/Datenwerk/posts/stan_glm01/stan_glm01.html)
2. [ReThink4e1](https://sebastiansauer.github.io/Datenwerk/posts/rethink4e1/rethink4e1)
2. [ReThink4e3](https://sebastiansauer.github.io/Datenwerk/posts/rethink4e3/rethink4e3)
2. [Kung-height](https://sebastiansauer.github.io/Datenwerk/posts/kung-height/kung-height)
2. [Pupil-size](https://sebastiansauer.github.io/Datenwerk/posts/pupil-size/pupil-size)
2. [IQ-Studentis](https://sebastiansauer.github.io/Datenwerk/posts/iq-studentis/iq-studentis)
2. [Priori-Streuung](https://sebastiansauer.github.io/Datenwerk/posts/priori-streuung/priori-streuung)
