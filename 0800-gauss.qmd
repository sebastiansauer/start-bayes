...
Wir interessieren uns also für den Mittelwert der Körpergröße (erwachsene Person^[Der Einfachheit halber gehen wir davon aus, dass Männer und Frauen im Schnitt gleich groß sind.]), $\mu$.
...
Wir nehmen an, dass die mittereln Größen, $\mu$, und die tatsächlichen Größen, $h_i$, normalverteilt sind und $\sigma$ halbseitig-normalverteilt ist (da notwendig positiv) ist:
...
#| fig-cap: Unser (erstes) Kung-Modell (m_kung)
#| fig-asp: 0.4
plots(p1, p2)
```
...
Zur Berechnung von `m_kung` nutzen wir jetzt dieses Mal aber *nicht* die Gittermethode (Bayes-Box),
...
Nennen wir das Modell `m_kung`^[*m* wie Modell und 4, weil das Modell in Kapitel 4 von @mcelreath2020 in ähnlicher Form berichtet wird, und 1 weil es unsere erste Variante dieses Modells ist.].
...
Für das Modell `m_kung` haben wir *keine* Prioris spezifiziert.
...
Im Modell `m_kung` haben wir auf die Priori-Werte der Voreinstellung von `rstanarm` vertraut.
Jetzt lassen wir mal unsere eigenen Priori-Werte einfließen,
in unserem zweiten Kung-Modell, `m42`.
...
:::callout-important
Vergleichen Sie die Parameterwerte von `m_kung` und `m42`! Was fällt Ihnen auf? Nichts?
...
Bisher haben wir untersucht, wie die Verteilung der *mittleren* Körpergrößen, 
$\mu$, laut unserem Modell aussehen könnte; wir haben uns also mit der Post-Verteilung
von $\mu$ beschäftigt.
Wir könnten aber auch an der Frage der Verteilung der *tatsächlichen* Körpergrößen, $h_i$,
laut Modell, interessiert sein: 
...
Wenn wir das Ergebnisobjekt unserer Analyse, `m_kung` in eine Tabelle (Tibble) umwandeln
und (die ersten paar Zeilen) betrachen, sehen wir diese Stichproben:
...
$\mu$ wird von Stan, der in Begriffen der Regressionsanalyse denkt, schnöde als `(Intercept)`
bezeichnet.
Wir könnten jetzt also für jede Zeile eine Normalverteilung berechnen. 
Und daraus zufällig eine Zahl ziehen.
Damit hätten wir dann unsere Verteilung von Körpergrößen laut Modell.
