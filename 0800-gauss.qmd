...
Aber bei jedem Start von R bzw. Ihrer GUI wie Positron oder RStudio müssen Sie die (benötigten!) Pakete starten.
...
```{r Post-Regression-3}
Kung_path <- paste0(
  "https://raw.githubusercontent.com/sebastiansauer/Lehre/",
  "main/data/Howell1a.csv")
...
[Datenquelle](https://raw.githubusercontent.com/sebastiansauer/2021-wise/main/Data/Howell1a.csv)^[<https://raw.githubusercontent.com/sebastiansauer/2021-wise/main/Data/Howell1a.csv>]
...
::: {.content-visible when-format="html"}
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/0800-gauss.html).
:::
:::
...
In diesem Kapitel befragen wir die Post-Verteilung für eine normalverteilte Zufallsvariable, nämlich die Körpergröße der !Kung San.
...
[Quelle](https://en.wikipedia.org/wiki/%C7%83Kung_people)^[<https://en.wikipedia.org/wiki/%C7%83Kung_people>]
...
describe_distribution(kung_erwachsen,
  centrality = "mean",   # "mean", "median", "MAP", "all"
  dispersion = TRUE,     # SD / MAD
  iqr = FALSE,            # Interquartilsabstand
  range = FALSE,          # Min/Max
  quartiles = FALSE,     # 25./75. Perzentil
  ci = NULL              # z.B. 0.95 für Konfidenzintervall
) |> 
  gt() |> 
...
Die Verteilungen lassen sich mit `plot_density` (aus dem R-Paket `{DataExplorer}`) erstellen, 
s. @fig-kung-dens.
...
![Mensch; Bildquelle: Wikimedia, Own alterations and File:SVG_Human_With_All_Organs.svg by Madhero88, CC BY-SA, 3.0](img/human.png){width="5%"}
...
In Worten: Wir halten es apriori zu 95 % für plausibel, dass die *durchschnittliche* Körpergröße der erwachsenen !Kung zwischen 138 cm und 218 cm liegt -- laut unserem Modell (unserer Apriori-Verteilung). Das ist eine Aussage über den unbekannten Mittelwert $\mu$, nicht über die Körpergröße einzelner Personen (dafür ist $\sigma$ zuständig).
...
[^0800-gauss-5]: Der Autor des zugrundeliegenden Lehrbuchs, Richard McElreath, gibt 178 cm als seine Körpergröße an.
...
2.  Apriori für $\mu$
3.  Apriori für $\sigma$
4.  Stan soll nicht so geschwätzig sein und alle seine Stichproben am Bildschirm zeigen, sondern nur das Wichtigste zurückmelden.
...
Betrachten wir die Post-Verteilungen von $\mu$ und von $\sigma$, s. @fig-kung3.
...
auf einen Bereich von etwa `r ci_low` bis `r ci_high` (95 %-ETI in der Voreinstellung^[https://easystats.github.io/parameters/reference/model_parameters.brmsfit.html]).
...
findet [hier](https://easystats.github.io/easystats/) einen Anfang.^[<https://easystats.github.io/easystats/>]
...
:::: {layout="[ 20, 80 ]"}
::: {#first-column}
![Hallo Leute, hier ist Stan!](img/Golem_hex.png){width="50%"}
:::

::: {#second-column}
Hier ein paar Kerninfos zu `stan_glm`:

-   *Stan*)^[<https://mc-stan.org/>] ist eine Software zur Berechnung von Bayesmodellen; das Paket `rstanarm` stellt Stan für uns bereit.
...
Mehr findet sich in der [Dokumentation von RstanArm](https://mc-stan.org/rstanarm/)^[<https://mc-stan.org/rstanarm/>].
...
Jetzt vertrauen wir die Wahl der Prioris Stan an.
...
#| tbl-cap: "Parameter von m_kung_neue_prioris mit Standard-Prioriwerten"
m_kung_neue_prioris <- 
  stan_glm(height ~ 1, 
           refresh = FALSE,  # bitte nicht so viel Ausgabe drucken
...
Wenn wir das Ergebnisobjekt unserer Analyse, `m_kung`, in eine Tabelle (Tibble) umwandeln und (die ersten paar Zeilen) betrachten,
...
p3 <- 
  ...
  geom_density(fill = "grey33") +
  scale_x_continuous(
    breaks = c(0, 178-3*height_sim_sd, 178, 178+3*height_sim_sd)) +
  scale_y_continuous(NULL, breaks = NULL) +
...
Sehr große Buschleute? `r round(anteil_großer_kung$prop[2], 2)*100` Prozent sind größer als 2 Meter. 
Das ist diskutabel, muss aber nicht zwangsläufig ein schlechter Prior sein.
