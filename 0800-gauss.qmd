...
Der Golem hört auf den Namen `rstanarm`.
...
m_kung <- 
  ...
  prior_aux = exponetial(1/8),
  refresh = 0,
  data = kung_erwachsen)  # <1>
...
Das Wesentliche: Unser Golem schätzt den Größenmittelwert der Kung auf ca. 155cm bzw. 
auf einen Bereich von etwa `r ci_low` bis `r ci_high` schätzt. 
...
### m_kung_mit_standard_prioris

Dann lassen wir `stan_glm()` (Stan) unser zweites Modell berechnen.[^0800-gauss-9] 
Dieses Mal lassen wir Stan die Priori-Werte selber bestimmen, s. @tbl-m_kung_neue_prioris-params.
