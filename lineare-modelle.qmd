...
vertical_norm <- function(x, y, k = 3, sigma_norm = 1) {
  
  norm_dens <-
    tibble(
      x_raw = seq(-k*sigma_norm, k * sigma_norm, length.out = 100),
      y_raw = dnorm(x_raw, mean = 0, sigma_norm) / dnorm(0, 0, sigma_norm) * 3,
      x = x_raw + x,
      y = y_raw - k * sigma_norm, 
      xend = x,
      yend = y_raw + k * sigma_norm)
...
plot_condition <- 
  ...
  geom_path(aes(y, x), data = vertical_norm(x = 40, y = 150), 
            color = "green")
```
...
Vergleichen wir die echten Werte für `height`, $y$, mit den von der PPV simulierten Werten für `height`, $y_{rep}$.
