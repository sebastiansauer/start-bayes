...
Wir nennen das Modell `m43a`^[Wer ist hier für die Namensgebung zuständig? Besoffen oder was?].
...
![Typischer Bayes-Nutzer, der ein Ungewissheitsintervall berechnet. Bildquelle: Easystats, bayestestR](img/bayesianMaster.jpg)
...
Die Verteilung der Größe bei einem Gewicht von 50kg ist weiter rechts (Richtung höhere Größe) zentriert, s. @fig-mu-at-50.
...
#| echo: false
#| label: fig-mu-at-50
#| fig-cap: Post-Verteilung der Größe (laut unserem Modell) bedingt auf ein Gewicht von 50 kg
mu_at_50 %>% 
  ggplot(aes(x = mu_at_50)) +
  geom_density() +
  scale_y_continuous(NULL, breaks = NULL) +
...
Noch eine andere Visualisierung, s. @fig-katzenaugen; je dicker die "Katzenaugen", desto mehr Stichproben (samples) liegen vor an der Stelle,
