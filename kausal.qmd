...
Berechnen wir das Modell `m1`; die Parameter stehen in @tbl-m1-hdi.
...
Berechnen wir die Vorhersagen für Dons neues Haus (mit den durch Mauern halbierten Zimmern), s. @tbl-m1-pred2a.
...
*Ohne* Kontrollieren der Konfundierungsvariablen: Regressionsmodell `y ~ x`, @fig-p-konf1.
...
*Mit* Kontrollieren der Konfundierungsvariablen: Regressionsmodell `y ~ x + group`, @fig-p-konf2.

```{r plot-konf2}
#| echo: false
#| fig-cap: "Keine Konfundierung von y und x!"
#| label: fig-p-konf2
p_konf2
```

@fig-p-konf2 zeigt korrekt, dass es keine Korrelation zwischen `x` und `y` gibt, wenn `group` kontrolliert wird.
Außerdem sieht man in @fig-p-konf2, dass es ein Kontrollieren der Variable `group` durch Aufnahme als Prädiktor in die Regressionsgleichung einem Stratifizieren entspricht (getrennte Berechnung der Regressionsgerade pro Gruppe).
...
`b`: bedrooms, `p`: price, `a` area (living area), s. @fig-km1.
...
`b`: bedrooms, `p`: price, `a` area (living area), s. @fig-m1-mediation.
...
Fazit: `km2` asst zu den Daten, aber wir sind nicht gerade beeindruckt vom Modell.
...
`b`: bedrooms, `p`: price, `a` area (living area), s. @fig-km3.
...
```{r}
#| fig-cap: "Zum Zusammenhang von Fleiss und Talent"
#| label: fig-m-eignung
m_eignung <-
...
weil sei die Wirkung von X auf Y "vermittelt" oder übeträgt.
