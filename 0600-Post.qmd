...
Die Bayesbox ist ein Weg, die Posteriori-Verteilung zu berechnen. Die Posteriori-Verteilung birgt viele nützliche Informationen.
...
$W=6$ Wasser, $N=9$ Würfen, bei Apriori-Indifferenz gegenüber den Parameterwerten. Und, sagen wir, $g=11$ Gitterwerten^[Die Anzahl *g* der Gitterwerte ist nicht Teil des Modells; die Anzahl der Gitterwerte entscheiden nur über die Genauigkeit der Post-Verteilung.] von 0 bis 1, in Schritten von 0.1, $0.1, 0.2, \ldots, 0.9, 1.0$. $\square$
:::
...
Abb. @fig-post1a zeigt die resultierende Bayesbox (s. @exm-globus69); vor allem ist die Post-Verteilung wichtig.
...
Voilà, die Post-Verteilung zu @exm-globus69 als Tabelle, die "Bayesbox": s. @tbl-post1.
...
Oder Sie starten das R-Paket `prada`, wo die Funktion wohnt:
...
2. Die Funktion verhält sich wie eine gewöhnliche Bayesbox: Bei `hyps` schreibt man die Hypothesen (bzw. Parameterwerte) auf. Bei `priors` die Priori-Werte und bei `liks` die Likelihoods der jeweiligen Hypothesen.
...
dann haben wir folgende drei Parameter zu schätzen: $\beta_0, \beta_1, \sigma$.
...
Kurz gesagt: Komplexere Bayes-Modelle können nicht mehr "einfach mal eben" ausgerechnet werden; 
...
Wenn Sie jetzt denken: "Warum machen wir das jetzt? Brauchen wir doch gar nicht!" -- Dann haben Sie Recht, für dieses Kapitel.
...
Jetzt die Post-Verteilung noch mal mit mehr Stichproben: $n=10^6$ Stichproben bei $g=101$ Gitterwerten aus der Posteriori-Verteilung, s. @fig-post-dk100.
...
Beispiel: Wenn der Parameterwert 0.6 eine Posteriori-Wahrscheinlichkeit von 0.27 hat, dann schreiben Sie 27 Zettel (von 100) mit dem Wert 0.6. $\square$
:::
...
Im *linken* Teildiagramm von @fig-p-vs-q fragen wir: "Welcher Wasseranteil wird mit einer gegebenen Wahrscheinlichkeit *p* nicht überschritten?" -- Wir suchen also einen Parameterwert (Quantil, q).
Im *rechten* Teildiagramm fragen wir: "Wie wahrscheinlich ist ein Wasseranteil von höchstens q?" -- Wir suchen also eine Wahrscheinlichkeit (p).
...
Er gruppiert die Stichprobe nach dem Prüfkriterium (hier: `p_grid < .5`, d.h. 
Wasseranteil höchstens 50%). 
Dann zählt er in jeder der beiden Teiltabelle 
die Zeilen und liefert diese zwei Zahlen dann zurück. $\square$
:::
...
Das 10%, 20%, ... 100%-Quantil^[d.h. die *Dezile*] (auf Basis von `bayesbox_g100_samples`) sind in @fig-quantiles-anim illustriert.
...
Noch einmal zum Globusversuch: Gehen wir von 3 Würfen mit 3 Mal Wasser (Treffer) aus; auf welche Wasseranteile (Parameterwerte) werden wir jetzt schließen? $\square$
:::
...
Intervalle höchster Dichte (Highest density Intervals, HDI oder HDPI) sind definiert als das *schmälste* Intervall, das den gesuchten Parameter enthält (in Bezug auf ein gegebenes Modell). $\square$
...
Im Globusversuch, Datensatz `samples_6w_9v`, s. @lst-post-sample. Sagen wir, wir haben 6 Treffer bei 9 Würfen erzielt.
