...
Aufgrund der Apriori-Indifferenz entsprechen die Posteriori-Wahrscheinlichkeiten den Likelihoods. Die höchste Wahrscheinlichkeit (d.h. Likelihood) hat derjenige Parameterwert, zu dem die Daten am besten passen, und das ist 6/9 = 2/3, d. @lst-bayes1.
$\square$
:::
::::
...
Entweder importiert ("sourct") man die Funktion `bayesbox` direkt von Github:
...
Sie könnnen es so installieren:
...
Erstellen wir uns also einen Tabelle mit Stichprobendaten aus der Posteriori-Verteilung (s. Dataframe `bayesbox_6w_9v `), s. @lst-post-sample.
...
Dann zählt er in jeder der beiden Teiltabelle 
...
Wir möchten also ziemlich sicher, was die Obergrenze an Wasser auf diesem Planeten ist.
...
```{r piplot}
#| echo: false
#| fig-cap: "Perzintilintervalle: Das Intervall umfasst die mittleren 80% der Wahrscheinlichkeit"
#| label: fig-eti-hdi
...
Das 10%, 20%, ...
100%-Quantil^[d.h. die *Dezile*] (auf Basis von `bayesbox_g100_samples`) sind in @fig-quantiles-anim illustriert.
...
Bei einem HDI sind die abgeschnitten Ränder nicht mehr gleich groß,
im Sinne von enthalten nicht (zwangsläufig) die gleiche Wahrscheinlichkeitsmasse. 
...
Im Globusversuch, Datensatz `samples_6w_9v`, s. @lst-post-sample.
...
Anstelle der Streuungsparameter für den gesuchten Parameter ist es aber üblicher, ein HDI oder PI für den gesuchten Paramenter anzugeben.
