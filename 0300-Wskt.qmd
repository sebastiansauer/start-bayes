...
@fig-baummuenz1 zeigt die möglichen Ausgänge (K wie Kopf und Z wie Zahl) dieses Zufallexperiments.

```{mermaid}
flowchart LR
 M[Sie werfen die Münze] --> K
 M --> Z
```

Zahl! Glück gehabt!^[?]$\square$
:::
...
:::{#def-multsatz}
### Multiplikationssatz für unabhängige Ereignisse
Die Wahrscheinlichkeit, dass zwei unabhängige Ereignisse $A$ und $B$ *gemeinsam* eintreten, beträgt

$Pr(A \cap B) = Pr(AB) = Pr(A) \cdot Pr(B)\square$.
:::
...
3. [mtcars-abhaengig_var3](https://datenwerk.netlify.app/posts/mtcars-abhaengig_var3/mtcars-abhaengig_var3)
