...
Wozu brauchen Sie dieses Kapitel? Im wirklichen Leben sind Aussagen (Behauptungen) so gut wie nie sicher:
...
Hier lernen Sie, wie man mit Unsicherheiten umgehen und trotzdem Aussagen treffen kann.
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/0300-Wskt.html).
:::
...
Oder probieren Sie die [App der Brown University](https://seeing-theory.brown.edu/basic-probability/index.html#section1)^[<https://seeing-theory.brown.edu/basic-probability/index.html#section1>], 
...
[Mit dieser App](https://www.geogebra.org/m/cbqee8h7)^[<https://www.geogebra.org/m/cbqee8h7>] können Sie Würfelwürfe simulieren und die Ausgänge dieses Zufallsexperiments beobachten. $\square$
:::
...
:::{#def-Ergebnisraum}
### Ergebnisraum
Die möglichen Ergebnisse eines Zufallsvorgangs fasst man als Menge mit dem Namen *Ergebnisraum* zusammen. 
...
@fig-baummuenz1 zeigt die möglichen Ausgänge (T wie Treffer (Party) und N (Niete, Lernen)) dieses Zufallsexperiments.
...
:::{#def-unm-sich}
### Unmögliches und sicheres Ereignis
Die leere Menge $\varnothing$ heißt das *unmögliche*, der Grundraum $\Omega$ heißt das *sichere Ereignis*. $\square$
:::
...
:::{#exm-sicher}
### Sicheres Ereignis
Nachdem der Würfel geworfen wurde, liegt eine Augenzahl zwischen 1 und 6 oben. $\square$
:::
...
:::{#def-vollereignis}
### Vollständiges Ereignissystem
Wird der Grundraum $\Omega$ vollständig in paarweise disjunkte Ereignisse zerlegt, 
...
:::{#def-macht}
### Mächtigkeit
Die Anzahl der Elementarereignisse eines Ereignisraums nennt man die Mächtigkeit (des Ergebnisraums).^[Die Menge aller Teilmengen einer Menge $A$ nennt man die *Potenzmenge* $\mathcal{P}(A)$, vgl. <https://de.wikipedia.org/wiki/Datei:Hasse_diagram_of_powerset_of_3.svg>.] $\square$
:::
...
![Zwei disjunkte Ereignisse, dargestellt durch überlappungsfreie Kreise](img/kreise_a_b_v2.svg){#fig-disjunkt width="25%" fig-align="center"}
...
Formaler ausgedrückt: Sei $Pr$ die Wahrscheinlichkeit, die das Ereignis $Z$ (Zahl) angibt, *gegeben* meiner Annahme, dass die Münze fair ist ($F$); sie hat also eine Wahrscheinlichkeit von 50 % (1/2 = .5) für $Z$. Dann kann man kurz schreiben:
...
Das Beispiel zeigt auch, dass die Wahrscheinlichkeit eines Ereignisses (wie $Z$) von Hintergrundinformationen (Annahmen, Daten, Evidenz, ...) abhängt.
Ohne zu sagen, auf welche Hintergrundinformationen wir uns beziehen (die des unbedarften Spielers oder die des Betrügers), ist es sinnvoll, eine Wahrscheinlichkeit anzugeben.
...
Es ist daher unvollständig zu sagen: "Morgen wird es mit einer Wahrscheinlichkeit von 70 % regnen."

Man müsste Hintergrundinformation (*Evidenz*, $E$) ergänzen, z. B. "gegeben meines Wissens zum Wettermodell X".

Also: Anstelle von $Pr(\text{Regen}) = .7$ sollte man besser schreiben: $Pr(\text{Regen|Wettermodell X})$. $\square$
:::
...
Jemand beobachtet bei sich Symptome, die für eine Hautkrebserkrankung $K$ typisch sind. Die Person sagt sich: "Ach, die Wahrscheinlichkeit für Hautkrebs liegt bei einem Promille. Kein Grund für Sorge." Formal: $Pr(K) = 0.001$.
...
:::{#def-laplace}
### Laplace-Experiment
Ein Zufallsexperiment, bei dem alle Elementarereignisse dieselbe Wahrscheinlichkeit haben, nennt man ein *Laplace-Experiment*, s. @thm-laplace. $\square$
:::
...
Und hier der Anteil von  `6` im Verlauf unserer Würfe, s. @fig-wuerfel-lln.


```{r}
#| label: fig-wuerfel-lln
...
wuerfel_tab |> 
  ...
  labs(x = "Nummer des Würfelwurfs",
       y = "Kumulierte Häufigkeit einer Sechs") +
...
![$A \cup B$: Vereinigung zweier Mengen](img/venn-union.svg){#fig-cup width=25%}
...
[Animation zur Vereinigung von Mengen](https://www.geogebra.org/m/GEZV4xXc#material/cmXR8fHN)^[<https://www.geogebra.org/m/GEZV4xXc#material/cmXR8fHN>]; Quelle: Geogebra, J. Merschhemke.
...
![$A \cap B$: Schnitt zweier Mengen](img/venn-intersection.svg){#fig-cap width=25%}
...
daher folgt hier eine Eselsbrücke, s. @fig-esel.

![Eselsbrücke für Vereinigungs- und Schnittmenge: In der "Tasse" (Vereinigung) bleibt mehr hängen als in der "Käseglocke".](img/ven_cap_cup.svg){#fig-esel width=55%}
:::
...
![$\bar{A}$: Komplement von $A$](img/venn-complement.svg){#fig-neg width=25%}
...
das genau aus den Elementarereignissen von $A$ besteht,
...
![$A \setminus B$: Alle Elemente von $A$, die nicht in $B$ sind.](img/venn-setminus.svg){#fig-setminus width=25%}
...
Nutzen Sie diese [Animation zu Mengenoperationen](https://seeing-theory.brown.edu/compound-probability/index.html)^[<https://seeing-theory.brown.edu/compound-probability/index.html>] als weitere Veranschaulichung.
...
So hat die Variable *Geschlecht eines Babys* die beiden Ausprägungen *Mädchen* und *Junge* mit den Wahrscheinlichkeiten $p_M = 48.8\%$ bzw. $p_J = 51.2\%$, laut einer Studie [@gelman2021]. $\square$
:::
...
Bei einer guten Stichprobe kann man die Kennwerte der deskriptiven Statistik
...
Einfach gesprochen veranschaulicht ein Balken- oder Histogramm eine Verteilung. Man unterscheidet Häufigkeitsverteilungen (s. Abb. @fig-2dice-sim) von Wahrscheinlichkeitsverteilungen (Abb. @fig-2dice-prob).
...
@fig-zv-stetig-groesse versinnbildlicht die stetige Zufallsvariable "Körpergröße", 
die (theoretisch, in Annäherung) jeden beliebigen Wert zwischen 0 und (vielleicht) 2 Meter annehmen kann.
...
Sie stehen an der Bushaltestelle und warten auf den Bus.
Langweilig.
...
:::
::: {.content-visible unless-format="html"}
![Wie groß ist die Wahrscheinlichkeit, zwischen 0 und 5 Minuten auf den Bus zu warten? 50 Prozent!](img/p_bus2.png){#fig-bus width="60%"}
:::
...
#| label: fig-p42
#| warning: false
#| out-width: !expr if (knitr::is_latex_output()) "60%" else "100%"
...
Die Wahrscheinlichkeitsdichte $f(x)$ gibt an, wie viel Wahrscheinlichkeitsmasse pro Einheit von $X$ an der Stelle $x$ ist. $\square$
:::
...
![Die Wahrscheinlichkeit, dass eine Zufallsvariable einen Wert in einem bestimmten Intervall annimmt, entspricht dem Inhalt der Fläche unter dem Graphen der Wahrscheinlichkeitsdichtefunktion.](img/flaeche_unter_kurve_v2.svg){#fig-wdichte-sinnbild width=33%}
...
- [prob-sicher-unmöglich](https://datenwerk.netlify.app/posts/prob-sicher-unm%C3%B6glich/)
- [indifferenz-p](https://datenwerk.netlify.app/posts/indifferenz-p/indifferenz-p.html)
...
quiz_wskt_files <- list(
  "exr/maechtigkeit-augensumme-schoice/maechtigkeit-augensumme-schoice.Rmd",
  "exr/vollstaendiges-ereignissystem-schoice/vollstaendiges-ereignissystem-schoice.Rmd",
  "exr/disjunkte-ereignisse-schoice/disjunkte-ereignisse-schoice.Rmd",
  "exr/kolmogorov-axiome-schoice/kolmogorov-axiome-schoice.Rmd",
  "exr/laplace-vs-frequentistisch-schoice/laplace-vs-frequentistisch-schoice.Rmd",
  "exr/bedingte-wahrscheinlichkeit-hintergrundwissen-schoice/bedingte-wahrscheinlichkeit-hintergrundwissen-schoice.Rmd",
  "exr/stetige-zv-dichte-schoice/stetige-zv-dichte-schoice.Rmd",
  "exr/logische-differenz-asymmetrie-schoice/logische-differenz-asymmetrie-schoice.Rmd",
  "exr/elementarereignis-muenzwurf-schoice/elementarereignis-muenzwurf-schoice.Rmd",
  "exr/verteilungsfunktion-wuerfel-schoice/verteilungsfunktion-wuerfel-schoice.Rmd"
)
```
...
#| message: false
#| results: asis
exams2forms(quiz_wskt_files, n = 1)
```
