...
Die formallogische Konzeption von Wahrscheinlichkeit sieht Wahrscheinlichkeit als Erweiterung der formalen Logik.^[Manchmal wird diese Art der Wahrscheinlichkeit auch *epistemologische* Wahrscheinlichkeit genannt.] 
In der formalen Logik ist ein Ereignis entweder *falsch* oder *wahr*. 
In der formallogischen Konzeption wird der Platz zwischen "falsch" (0) und "richtig" (1
als die (nicht sichere) Wahrscheinlichkeit $0<p<1$, gesehen [@briggs_uncertainty:_2016], s. @fig-prob-logic.
...
Man beachte, dass die beiden Ergebnisse disjunkt sind, s. @fig-add-disjunkt.
...
:::{#def-add-disjunkt}
### Additionssatz für disjunkte Ereignisse
Die Wahrscheinlichkeit, dass mindestens eines der beiden Ereignisse eintritt, ist die Summe der Einzelwahrscheinlichkeiten. $\square$
:::
...
:::{#def-gem-wskt-abh}
Die Wahrscheinlichkeit, dass zwei abhängige Ereignisse $A$ und $B$ *gemeinsam* eintreten, ist gleich dem Produkt der Wahrscheinlichkeit von $A$ und der bedingten Wahrscheinlichkeit von $B$ gegeben $A$, s. @eq-gem-wskt-abh. $\square$
:::

$$Pr(A \cap B) = Pr(A) \cdot Pr(B|A)$$ {#eq-gem-wskt-ab
...
Man kann  die "Gleichung drehen": $$P(❄️ \text{ und } ⛈️) = P(⛈️ \text{ und } ❄️$${#eq-kalt-regen-emoji2} $\square$
:::
...
Drei Maschinen ($M_1, M_2, M_3$) produzieren einen Artikel. 
