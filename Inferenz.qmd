...
::: {#fig-xy layout-ncol=2}

```{mermaid}
%%| fig-cap: Sinnbild eines statistischen Modells
...
```{mermaid}
%%| fig-cap: Sinnbild eines statistischen Modells, mit zwei Inputvariablen (Ursachen)
...
Sinnbild zweier statistischer Modelle 
:::
...
Punktschäter beinhalten *keine Angabe* der Schätz(un)gen auigkeit, s. Abb. @fig-punktschaetzer, links.

```{r}
#| echo: false
#| fig-cap: "Eine Punktschätzung und ihre Ungewissheit"
...
plots(plot1, plot2, n_rows = 1,
      title = c("Eine Punktschätzung mittels einer Regressionsanalyse",
                "Ungewissheit in der Punktschätzung einer Regressionsanalyse"))
```
...
In Abb. @fig-punktschaetzer, rechts, ist die Ungewissheit in den Regressionskoeffizienten visualisiert: Wie sicher sind wir uns zur Stärke des Zusammenhangs von X und Y?
...
Dieser Befund läßt sich so interpretieren: "Laut Modell liegt der gesuchte Anteil mit einer Wahrscheinlichkeit von 95% im Bereich von 44 bis 44 Prozentpunkten."
