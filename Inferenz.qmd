...
::: callout-note
Ziele exsitieren nicht "in echt" in der Welt. 
...
### Deskriptiv- und Inferenzstatistik gehen Hand in Hand
 ...
 Für jede Statistik (Kennzahl von Stichprobendaten) kann man die Methoden der Inferenzstatistik verwenden, s. Tabelle @tbl-kennwerte.
...
Noch allgemeiner ist vom häufig am Zusammenhang von `X ` und `Y` interessiert, s. Abb. @fig-xy, die ein Sinnbild eines statistischen Modells widergibt.

```{mermaid}
%%| label: fig-xy
%%| fig-cap: Sinnbild eines stastistischen Modells
...
Okay, ein statistisches Modell kann natürlich komplexer sein, z.B. wie in Abb. @fig-xy2 dargestellt.



```{mermaid}
%%| label: fig-xy2
%%| fig-cap: Sinnbild eines stastistischen Modells, mit zwei Inputvariablen (Ursachen)
...
![One regression](img/einring.jpg){width="50%"}
...
![Common statistical tests as linear models](https://lindeloev.github.io/tests-as-linear/linear_tests_cheat_sheet.png){#fig-lindeloev}
...
Punktschäter beinhalten *keine Angabe* der Schätz(un)gen auigkeit, s. Abb. @fig-punktschaetzer.

```{r}
#| echo: false
#| fig-cap: Eine Punktschätzung
#| label: fig-punktschaetzer
#| message: false
#| fig-asp: 0.5
...
In Abb. @fig-ungewiss1 ist die Ungewissheit in den Regressionskoeffizienten visualisiert: Wie sicher sind wir uns zur Stärke des Zusammenhangs von X und Y?

```{r}
#| echo: false
#| fig-cap: Ungewissheit in den Regressionskoeffizienten
#| label: fig-ungewiss1
#| message: false
...
Auch wenn wir uns *sicher* im Hinblick auf die Regressionsgewichte in Abb. @fig-ungewiss1 *bliebe eine Restungewissheit*:
Unsere Schätzungen wären nicht sicher, nicht fehlerfrei.
...
Die Posteriori-Verteilung beantwortet uns die Frage: "Wie wahrscheinlich ist die Forschungshypothese, jetzt, nachdem wir die Daten kennen, auf Baiss unseres Modells?"
...
![Frequentist wettet mit Bayesianer](https://imgs.xkcd.com/comics/frequentists_vs_bayesians_2x.png){width=50%}
