...
Das schwierigste ist dabei, nicht zu vergessen, dass `d3` die Tabelle mit zentriertem Prädiktor ist, nicht `kung_erwachsen`.
...
Modelldefinition von `m43a` in R
:::
...
estimate_expectation(m43a, by = "weight_c") |> plot()  # aus {easystats}
...
estimate_expectation(m43a, by = "weight_c") %>% plot()
```
