...
wo die Körpergröße auf das Gewicht zurückgeführt wird; also ein sehr eingängiges Modell.  
...
Wie aus @tbl-kung1 abzulesen ist, beträgt das mittlere Körpergewicht (`weight`) liegt ca. 45kg (sd ca 7 kg).
...
Allerdings sind Korrelationen nur für zwischen zwei Variablen definiert; 
...
Je schwerer (je höher das Gewicht einer Person), desto größer (dest höher ihr Wert in der Körpergröße).
...
#| fig-cap: 'Das Zentrieren ändert die Verteilungsform nicht, sondern "schiebt" die Verteilung nur zur Seite. Der resultierende Mittelwert ist dann Null. Dier zentrierten Werte zeigen, wie weit ein Messwert vom Mittelwert entfernt ist (hier in der Einheit kg).'
...
Anders formuliert: "Um wiele Zentimeter Körpergröße unterscheiden sich zwei erwachsene Kung,
...
- Die Körpergrößen sind gleich mit um das Regressionsgewicht gewichtete Wert des jeweiligen Körpergewichts.
...
Dazu addieren wir zum Wert des Achsenabschnitts (`intercept`) das Fünffache des Regressionsgewichts, $\beta_1$ (`weight_c`): `mu_at_50 = (Intercept)m + 5 * weight_c`.
