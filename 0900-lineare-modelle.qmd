...
kung_path <- paste0(
  ...
  "Lehre/main/data/Howell1a.csv")  # <1>
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/0900-lineare-modelle.html).
:::
:::
...
#| echo: false
#| label: tbl-kung1
#| tbl-cap: "Verteilung der (metrischen) Variablen im !Kung-Datensatz"
describe_distribution(kung_erwachsen) |> 
  as.data.frame() |>
  select(-Skewness, -Kurtosis, -n_Missing, -IQR) |>
  knitr::kable()
```
...
Betrachten wir nun die Verteilung der *kategorialen* Variablen des Datensatzes:
...
<!-- PDF/Print: panel-tabset innerhalb eines #fig-Div erzeugt einen LaTeX-Float,
der wegen des zweiten (redundanten) ggpubr-Tabs zu gross fuer eine Seite wird
und dadurch verloren geht/springt. Fuer den Druck genuegt eine der beiden
aequivalenten Darstellungen als normale Abbildung. -->
...
#| echo: false
#| label: fig-kung-zshg
#| fig-cap: "Der Zusammenhang zwischen Gewicht (X) und Größe (Y)"
kung_erwachsen |>
...
$$
 ...
 $\text{weightcentered}_i$ ist der zentrierte Gewichtswert (`weight_c`) der $i$ten Beobachtung, also einer !Kung-Person (Zeile $i$ im Datensatz).
Lies  etwa so:

>    "Der vorhergesagte mittlere Wert der Körpergröße, $\mu_i$, der $i$ten Person berechnet sich als Summe von $\beta_0$ und $\beta_1$ mal  $\text{weightcentered}_i$".


 $\mu_i$ ist eine lineare Funktion von `weight_c`.
 $\beta_1$ gibt den Unterschied in `height` zweier Beobachtung an, die sich um eine Einheit in `weight_c` (also auch in `weight`) unterscheiden (Steigung der Regressionsgeraden).
 $\beta_0$ gibt an, wie groß $\mu$ ist, wenn `weight_c` Null ist, d.h. beim Durchschnittsgewicht (Achsenabschnitt, engl. intercept).
...
```{r Post-Regression-befragen-10, echo = FALSE}
kung_zentriert |>
  ggplot(aes(x = weight_c,
             y = height)) +
  geom_point() +
...
```{r Post-Regression-befragen-11-ersten-1000, echo = FALSE}
kung_zentriert |>
  ggplot(aes(x = weight_c,
             y = height)) +
  geom_point() +
...
```{r Post-Regression-befragen-11-die-ersten-1000, echo = FALSE}
kung_zentriert |>
  ggplot(aes(x = weight_c,
             y = height)) +
  geom_point() +
...
<!-- PDF/Print: panel-tabset innerhalb eines #fig-Div erzeugt einen zu grossen
LaTeX-Float (vier Tabs mit je einem vollen Plot). Fuer den Druck stattdessen
eine fig-subcap-Abbildung (10/100/1000 Stichproben) mit 2 Spalten
(layout-ncol: 2) statt 3 -- bei mehr als 2 Diagrammen pro Zeile werden die
Einzelplots im Druck zu klein/unleserlich; der vierte ("1000000"-Scherz-)Tab
hat ohnehin kein Bild. -->

::: {.content-visible unless-format="html"}

```{r}
#| echo: true
#| label: fig-ungewissheit-kung
#| fig-cap: "Ungewissheit der Post-Verteilung, dargestellt anhand von Stichproben aus der Post-Verteilung"
#| layout-ncol: 2
#| fig-subcap:
#|   - "10 Stichproben"
#|   - "100 Stichproben"
...
kung_zentriert |>
  ...
  geom_point() +
  geom_abline(
    data = m_kung_starkes_beta1_post |> slice_head(n = 10),
    aes(slope = weight_c, intercept = `(Intercept)`),
    alpha = .3)
...
kung_zentriert |>
  ...
  geom_point() +
  geom_abline(
    data = m_kung_starkes_beta1_post |> slice_head(n = 100),
    aes(slope = weight_c, intercept = `(Intercept)`),
    alpha = .1)
...
kung_zentriert |>
  ...
  geom_point() +
  geom_abline(
    data = m_kung_starkes_beta1_post |> slice_head(n = 1e3),
    aes(slope = weight_c, intercept = `(Intercept)`),
    alpha = .01)
```
...
[Quelle](https://easystats.github.io/bayestestR/articles/bayestestR.html)^[<https://easystats.github.io/bayestestR/articles/bayestestR.html>] $\square$
:::
...
m_kung_gewicht_c_post |> 
  mutate(mu45 = `(Intercept)`,
         mu40 = mu45 - 5 * weight_c,
         mu50 = mu45 + 5 * weight_c,
         mu55 = mu45 + 10 * weight_c)


m_kung_gewicht_c_post_long <-
  m_kung_gewicht_c_post |>
  select(mu40, mu45, mu50, mu55) |>
...
Die Verteilung der mittleren Größe bei einem Gewicht von 50 kg ist weiter "rechts" (Richtung höhere Größe) lokalisiert, s. @fig-mu-at-50.
Dazu addieren wir zum Wert des Achsenabschnitts (`intercept`) das Fünffache des Regressionsgewichts, $\beta_1$ (`weight_c`): `mu50 = (Intercept) + 5 * weight_c`.
...
Man erhält es mit dem Befehl `mu50 |> eti(ci = .9)`
