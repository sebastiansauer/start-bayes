...
ist das Testen der Hypothese "kein Effekt" (Null Effekt), man spricht vom *Nullhypothesen testen*. $\square$
:::
...
Wie groß ist der Schätzbereich für den Effekt des Parameter "Geschlecht" auf das Gewicht von Pinguinen? 
...
Nimmt man (praktisch) keinen Unterschied/Zusammenhang/Effekt an, spricht man von einer *Nullhypothese* (bzw. "Praktisch-Null-Hypothese"): $H_0$.
...
#| echo: false
#| fig-align: "center"
#| fig-cap: "Die Entscheidungsregeln zum ROPE illustiert [@kruschke2018]."
...
rope(m_penguins_species, range = c(-100, 100))
...
rope(m_penguins_species, range = c(-100,100), ci = .89, ci_method = "ETI")
```
...
$R^2$ und $\sigma$ sind negativ assoziert: In einem Datensatz mit mit hohem $R^2$ ist $\sigma$ gering und umgekehrt.
Beide Koeffizienten berechen sich auf Basis der Vorhersagefehler.
