...
<!-- TODO hier keine überschriften der Ebene 3 -->
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/1050-Schaetzen-Testen.html).
:::
:::
...
also als falsch (falsifiziert) betrachten oder zumindest hat die Glaubwürdigkeit der Hypothese gelitten.
...
:::{.callout-important}
Eine gängige Variante des Hypothesentestens^[vor allem in der Frequentistischen Statistik]
...
a) *Punktschätzung*: Das Schätzen eines einzelnen Parameterwerts, sozusagen ein "Best Guess"
...
Wie groß ist der Schätzbereich für den Effekt des Parameters "Geschlecht" auf das mittlere Gewicht von Pinguinen?
...
:::{#exm-param-hyptest}
### Parameterschätzen als gerichteter Hypothesentest
...
Anders als beim Testen einer *Null*hypothese (ist ein Parameter bzw. eine Differenz gleich Null?) prüfen wir hier eine *gerichtete* Hypothese: Ist die Differenz $d$ größer oder gleich Null?
...
Der Unterschied zwischen zwei Mittelwerten, $d$, 
...
zählen wir wie gewohnt den Anteil der Stichproben
in der Post-Verteilung für die UV `sexmale`, 
...
das heißt, weibliche Tiere sind leichter bzw. männliche Tiere schwerer (im Durchschnitt).
...
Einfacher zeigt uns `parameters(m_penguins_sex)`, 
wie groß die Wahrscheinlichkeit für $\beta_1 > 0$ ist,
nämlich anhand des Koeffizienten `pd`, s. @tbl-m-penguins-sex-params.

Aber das Auslesen der Post-Verteilung erlaubt uns auch, 
andere Hypothesen zu prüfen,
etwa die Wahrscheinlichkeit der Hypothese, 
dass der Gewichtsunterschied zwischen 
...
[Quelle: Imgflip Meme Generator](https://imgflip.com/i/5v5531)^[<https://imgflip.com/i/5v5531>]
...
#| echo: false
#| fig-align: "center"
#| fig-cap: "Die Entscheidungsregeln zum ROPE illustriert [@kruschke2018]; A: Verwerfen der ROPE-Hypothese; B und C: Akzeptieren; D-F: Unklare Datenlage, keine Entscheidung zur ROPE-Hypothese"
...
Das ROPE durchkreuzt die "Berge" der Posteriori-Verteilung für *Chinstrap* deutlich.
...
*Gentoo* hingegen wird vom Rope nicht durchkreuzt,
...
plot(rope(m_penguins_species, range = c(-200, 200))) + 
  scale_fill_okabeito()  # schönes Farbschema
```
...
wenn man möchte: `rope(m_penguins_species, range = c(-500,500), ci = .89, ci_method = "HDI")`.
Die ROPE-Grenzen sind in @tbl-rope-species-rope-fine zu sehen.
...
-- ähem -- subtilem Humor lieber die 89 als die 95. 🤷♂️ ] sich im Rope befindet.
...
C) Es gibt keinen Einfluss des Robo-Professors auf die Lernleistung der Studierenden.
...
Für die Spezies *Gentoo* wurde ein substanzieller Gewichtsunterschied zur Referenzgruppe, *Adelie*,
...
Eine Abschätzung zur Belastbarkeit der Modellergebnisse liefern Kennwerte der Modellgüte.
...
Da kann es helfen, eine zusammenfassende Beurteilung zur Präzision, oder allgemeiner zur Güte des Modells, zu erhalten.
Im Folgenden wird eine von mehreren gebräuchlichen und sinnvollen Kennzahlen vorgestellt, $R^2$.
...
- Höhere Werte von $R^2$ bedeuten, dass das Modell die Daten besser erklärt.
...
In einem Datensatz mit hohem $R^2$ ist $\sigma$ gering und umgekehrt.
...
so dass Sie in Ruhe Ihre offenen Fragen und diejenigen Ihrer Kommilitoninnen und Kommilitonen
...
4. [rope2a](https://sebastiansauer.github.io/datenwerk/posts/rope2a/index.html)
5. [penguins-rope](https://sebastiansauer.github.io/datenwerk/posts/penguins-rope/index.html)
