...
Bayes' Theorem (@eq-bayes3) fragt nach $Pr(H|D)$:
...
Die Evidenz berechnet sich als Summe der Likelihoods für alle Parameterwerte $H_i$, d.h. als die totale Wahrscheinlichkeit von $D$, s. @eq-evidenz, vgl. auch @def-totwskt:

$$\begin{aligned}
Pr(D) = \sum_{i=1}^n Pr(D|H_i) \cdot Pr(H_i)
\end{aligned}$${#eq-evidenz} 

Die verschiedenen Parameterwerte kann man auch als die verschiedenen Hypothesen $H_i$ auffassen.
Falls es nur zwei Hypothesen bzw. Parameterwerte gibt, vereinfacht sich @eq-evidenz zu @eq-evidenz2.

$$\begin{aligned}
Pr(D) = Pr(D|H_1) \cdot Pr(H_1) + Pr(D|H_2) \cdot Pr(H_2)
\end{aligned}$${#eq-evidenz2} 
:::
