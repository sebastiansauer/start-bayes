...
Die zugehörige Wahrscheinlichkeit der Daten unter Annahme einer Hypothese nennt man die^[oder den?] *Likelihood^[etwa: "Mutmaßlichkeit"].*
...
hier ist Pseudocode für Ihre Anwendung:
...
Wir fassen das allgemeine Globusmodell so zusammen, s. @eq-globus1.
...
Unser Vorab-  bzw. Apriori-Wissen zu $p$ sei, dass uns alle Werte gleich ("uniform") plausibel erscheinen:
...
$$Pr(H|D) =\frac{Pr(D\cap H)}{Pr(D)} =  \frac{\overbrace{Pr(H)}^\text{Apriori-Wahrscheinlichkeit} \cdot \overbrace{Pr(D|H)}^\text{Likelihood}}{\underbrace{Pr(D)}_\text{Evidenz}}$$ {#eq-bayes1}
...
Die Evidenz berechnet sich als Summe der Likelihoods über alle Ausprägungen von $D$, d.h. als die totale Wahrscheinlichkeit von $D$, vgl. @def-totwskt:
