...
### Ein Regressionsmodell ist aus der kleinen Welt

Ein wissenschaftliches Modell, etwa auf Basis eines Regressionsmodell ist Teil der *kleinen* Welt.
Man muss sich bei der Interpretation eines Regressionsmodell vor Augen halten: "Die Ergebnisse des Modell sind nur richtig unter der Annahme, dass sich der Zusammenhang X und Y durch eine Gerade beschreiben lasssen und unter der Annahme, dass meine Daten in Ordnung sind." $\square$
:::
...
Analog können wir uns vorstellen, 11 Wissenschaftler haben jeweils eine andere Hypothese zum Wasseranteil, $\pi$, der Erde. 
...
### Was ist die Wahrscheinlichkeit von 6 mal Wasser bei 9 Würfen?

Wie wahrscheinlich ist es, einen bestimmten Wasseranteil, z.B. 6 Treffer (bei 9 Würfen) zu erhalten, 
wenn man eine bestimmte Hypothese (einen bestimmten Wasseranteil, z.B. 90%) annimmt?
Diese Wahrscheinlichkeit nennt man die *Likelihood*, $L$.
...
Lassen Sie uns im Folgenden die Wahrscheinlichkeit ($Pr$), $W$ mal Wasser und $L$ mal Land zu beobachten, wenn die Wahrscheinlichkeit für Wasser $\pi$ beträgt, so bezeichnen: $Pr(W|\pi, n)$.
Diese Wahrscheinlichkeit kann man im Fall des Globusversuchs mit der *Binomialverteilung* berechnen.

Möchte man die Wahrscheinlichkeit ansprechen für das Ereignis "6 mal Wasser und 3 mal Land, 
wenn wir von einem Wasseranteil von 70% ausgehen", so würden wir kurz schreiben:  $Pr(W=6 | \pi=.7, n=9)$.
...
%%| echo: false
%%| label: fig-binom1a
%%| fig-cap: "Wir werfen den Globus (oder eine Münze) 3 Mal. Die Knoten sind der Übersicht halber mit fortlaufenden Buchstaben (von A bis P) bezeichnet. Das Ergebnis 'Wasser' ist *1* und das Ergebnis 'Land' mit *0* abgekürzt dargestellt."
flowchart TD
  A[A - Start] -. 1/2 .-> B[B - 0]
  A -. 1/2 .-> C[C - 1]
  B -. 1/2 .-> D[D - 0]
...
@fig-bin-klein zeigt die Binomialverteilung $X \sim Bin(n=9, \pi = 1/2)$:
...
:::{#exr-globus}
### Peer Instruction: Welche Anzahl von Wasser ist am plausibelsten?

Wir führen wieder den Globusversuch durch ($\pi=.7$), mit 9 Würfen.
Welches Ergebnis ist am plausibelsten?
...
Die Posteriori-Verteilung ist das Ergebnis des Bayes-Updates.
...
Die Posteriori-Wahrscheinlichkeit, $Pr(H|D)$,
ist schwerer zu bestimmen, aber interessanter.
Man könnte also sinnbildlich sagen,
das Bayes-Theorem ist eine "Maschine", die die Priori-Wahrscheinlichkeit zusammen mit 
der Likelihood zur Posteriori-Wahrscheinlichkeit "umbaut".
...
$$Pr(H|D) = \frac{ Pr(H) \cdot Pr(D|H) }{Pr(D)}\quad \square$$
:::
...
also $Pr(H|D)$.
Also zeigt Bayes' Theorem nichts anderes als eine normale bedingte Wahrscheinlichkeit.
...
Die Evidenz berechnet sich als Summe der Likelihoods für alle Parameterwerte $H_i$, d.h. als die totale Wahrscheinlichkeit von $D$, s. @thm-evidenz, vgl. auch @def-totwskt. $\square$
:::
...
<!-- Gesucht ist die Wahrscheinlichkeit einer Hypothese gegeben einer bestimmten Datenlage, $Pr(H|D)$, s. @thm-bayes5 -->

<!-- $$Pr(H|D) = \frac{Pr(D|H) \cdot Pr(H)}{Pr(D)} = \frac{\text{Likelihood}  \cdot \text{Priori-Wahrscheinlichkeit}}{\text{Evidenz}}$${#thm-bayes5} -->

Schauen wir uns die Bestandteile von Bayes' Theorem (@thm-bayes3) noch etwas näher an:
...
Die *unstandardisierte* Posteriori-Wahrscheinlichkeit $Pr_{\text{unPost}}$ ist einfach das Produkt von 
Likelihood und Priori, s. @thm-unpost.

::: {#thm-unpost}

### Unstandardisierte Posteriori-Wahrscheinlichkeit 

$$Pr_{\text{unPost}} = L \times \text{Priori}\quad \square$$
:::
...
### Standardisierte Posteriori-Verteilung

$$\text{Posteriori} = \frac{\text{Likelihood} \times \text{Priori}}{\text{Evidenz}}\quad \square$$
:::
...
## Bayes berechnen mit mit der Bayesbox
...
$$\text{Post}_{\text{unstand}} = \text{Priori} \cdot \text{Likelihood} = Pr(H) \cdot Pr(H|D)$${#eq-postunstand}
...
der unstandardisierten Post-Wahrscheinlichkeiten, s. @thm-poststand-allgemein.

::: {#thm-poststand-allgemein}

### Posteriori-Verteilung 2

$$\text{Post} = \frac{\text{Post}_{\text{unstand}}}{\text{Evidenz}} = \frac{Pr(H) \cdot Pr(H|D)}{Pr(D)}\quad \square$$
:::

Dabei haben wir die Priori-Wahrscheinlichkeit für alle Parameterwerte als gleich angenommen, da wir keinerlei Vorwissen hatten, $Pr(H_i) = 1/11$.
...
Auf einer Party: Unterhalten sich fünf Studis über das Bayesmodell. Einer hat Unrecht, die anderen Recht. 
...
B) Sind alle Hypothesen apriori gleich wahrscheinlich, dann hat die Hypothese mit dem höchsten Likelihood aposteriori auch die höchste Wahrscheinlichkeit.
...
D) Die unstandardisierte Posteriori-Wahrscheinlichkeit ist gleich der standardisierten mal einen Faktor $k$.
...
Dann lösen wir nach P$(H|D)$ auf, s. @thm-bayes3.
...
### Bayes' Theorem für zusammengesetzte Hypothesen
