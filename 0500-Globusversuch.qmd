...
::: {.content-visible when-format="html"}
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/0500-Globusversuch.html).
:::
:::
...
[Quelle: Ernst Ravenstein, Wikimedia, Public Domain](https://commons.wikimedia.org/wiki/File:RavensteinBehaim.jpg)^[<https://commons.wikimedia.org/wiki/File:RavensteinBehaim.jpg>]
...
[Bildquelle: Mikoláš Aleš, Wikimedia, Gemeinfrei](https://de.wikipedia.org/wiki/Golem)^[<https://de.wikipedia.org/wiki/Golem>]

[Der Golem von Prag](http://www.prague.net/golem)^[<http://www.prague.net/golem>], die Legende einer vom Menschen geschaffenen Kreatur mit gewaltiger Kraft, die Befehle wörtlich ausführt, s. @fig-golem-prag.
Die Geschichte besagt, dass ein Rabbi mit Zauberkräften den Golem aus Lehm erschuf, um die jüdische Bevölkerung der Stadt zu schützen.
...
!["Yeah, ich bin ein Golem!" -- Bildquelle: Klara Schaumann, abgebildet mit freundlicher Genehmigung der Autorin](img/Golem_hex.png){width=25%}
...
Vergleichen wir die kleine Welt unseres Modells (@tbl-klein-gross),
...
Um möglichst wenig schreiben zu müssen, schreiben wir für "angenommener Wasseranteil auf der Erdoberfläche" kurz $p$ oder $\pi$ (p wie proportion, Anteil). $\square$
:::

![Die Erde. Schön! Und mit viel Wasser, ca. 70 % der Erdoberfläche sind mit Wasser bedeckt.
Bildquelle: NASA "Blue Marble" (Apollo 17, 1972) -- gemeinfrei, kommerziell nutzbar ohne Attribution-Pflicht.](img/erde_freigestellt.png){#fig-erde width="10%" fig-align="center"}
...
Um die Forschungsfrage zu beantworten, werfen Sie einen Globus-Ball in die Luft und fangen ihn wieder auf.
...
haben. (Warum gerade 9 Mal? Tja, dann hat das Handy geklingelt... 
Auch in wissenschaftlichen Versuchen ist (leider?) nicht immer alles genau geregelt.)
...
:::{#exr-globe1}
### Der Globusversuch
Besorgen Sie sich einen Globus (zur Not eine Münze) und stellen Sie den Versuch nach! $\square$
:::
...
Sie arbeiten für eine Weltraumbehörde.
...
:::{#def-priori}
### Apriori-Verteilung
Für jede Hypothese haben wir ein Vorab-Wissen, das die jeweilige Plausibilität der Hypothese angibt: *Apriori-Verteilung*^[Anstatt von *Apriori-Verteilung* liest man auch *Priori-Verteilung* oder *Prior*.]. $\square$
:::
...
^[Die sog. "iid-Annahme", *i*ndependently and *i*dentically distributed: Jeder Wurf des Globusballes ist eine Realisation der gleichen Zufallsvariablen.
...
1. Die *Likelihood* (die Wahrscheinlichkeit der Daten unter Annahme der Hypothese), s. @eq-dbinom1
2. Die *Apriori-Verteilung(en)* (die Wahrscheinlichkeit der Hypothese vor den Daten), s. @eq-prior-unif-globus
...
Wie in @def-post1 eingeführt, quantifiziert die Aposteriori-Verteilung (kurz: "Post-Verteilung") unser Wissen zu den Parameterwerten *nach* Kenntnis der Daten und aufbauend auf unserem Vorwissen (Apriori-Wissen).
Sie ist das Ergebnis des Bayes-Updates; man bezeichnet sie kurz mit $Pr(H|D)$.
...
#| echo: false
#| label: fig-post3
#| fig-cap: "Prior mal Likelihood = Post"
...
Man sieht: Am Anfang, apriori, also bevor wir die Daten kennen,
...
```{r}
#| echo: false
#| label: fig-lernen-golem
#| fig-cap: "Unser Golem lernt"
...
Da wir letztlich eine Wahrscheinlichkeitsverteilung bekommen möchten,
teilen wir jeden Aposteriori-Wert durch die Summe aller Aposteriori-Werte. 
Dadurch ist garantiert, dass sich die Aposteriori-Werte zu eins aufaddieren.
...
Um zur *standardisierten* Aposteriori-Wahrscheinlichkeit zu gelangen,
...
$$\text{Post} = \frac{\text{Post}_{\text{unstand}}}{\text{Evidenz}} = \frac{Pr(H) \cdot Pr(D|H)}{Pr(D)}$${#eq-poststand-allgemein}
...
:::{#exr-priori-change}
Was wohl mit *Post* passiert, wenn wir *Apriori* ändern? $\square$
:::
...
::: callout-tip 
Wenn Sie auf einen Prozentwert für $W$ tippen müssten, welchen würden Sie nehmen, 
...
Der Globusversuch ist kein prototypisches Beispiel für Statistik in der Praxis, zumindest nicht auf den ersten Blick.
Er hat aber den Vorteil, dass es ein einfaches, gut greifbares Beispiel ist, und damit zum Lernen gut geeignet ist.
...
Das ["Bayes-Paradox-Video" von 3b1b](https://youtu.be/lG4VkPoG3ko) präsentiert eine gut verständliche Darstellung des Bayes-Theorems aus einer zwar nicht gleichen,
...
Gegeben sind die Wahrscheinlichkeiten, dass Maschine $i$ das Teil produziert hat, $Pr(M_i)$. Außerdem sind die Wahrscheinlichkeiten, dass das Teil Ausschuss ist, $Pr(A|M_i)$, bekannt.
...
%%| label: fig-tot-wskt2
%%| fig-width: 4.5
%%{init: {'flowchart': {'diagramPadding': 30, 'padding': 30}}}%%
flowchart LR
  A[Start] ==>|0.60|B[M1]
  A --->|0.10|C[M2]
  A --->|0.30|D[M3]
...
### Bayes' Theorem als bedingte Wahrscheinlichkeit
...
2. [kekse03](https://datenwerk.netlify.app/posts/kekse03/index.html)
...
```{r quiz-globusversuch-setup}
#| include: false
library(exams2forms)

quiz_globusversuch_files <- list(
  "exr/kleine-grosse-welt-schoice/kleine-grosse-welt-schoice.Rmd",
  "exr/bayes-begriffe-apriori-likelihood-post-schoice/bayes-begriffe-apriori-likelihood-post-schoice.Rmd",
  "exr/likelihood-globusversuch-berechnung-schoice/likelihood-globusversuch-berechnung-schoice.Rmd",
  "exr/bayesbox-standardisierung-schoice/bayesbox-standardisierung-schoice.Rmd",
  "exr/evidenz-maschinen-schoice/evidenz-maschinen-schoice.Rmd",
  "exr/bayes-theorem-maschinen-schoice/bayes-theorem-maschinen-schoice.Rmd",
  "exr/evidenz-funktion-schoice/evidenz-funktion-schoice.Rmd",
  "exr/bayes-post-aussagen-schoice/bayes-post-aussagen-schoice.Rmd",
  "exr/post-bei-uniformer-priori-schoice/post-bei-uniformer-priori-schoice.Rmd",
  "exr/bayesbox-post-berechnung-schoice/bayesbox-post-berechnung-schoice.Rmd"
)
```

```{r quiz-globusversuch}
#| echo: false
#| message: false
#| results: asis
exams2forms(quiz_globusversuch_files, n = 1)
```
