...
Zuerst hat er ein Vorwissen zum Wasseranteil, die dazugehörige Wahrscheinlichkeitsverteilung nennt man *Priori-Verteilung* (s. @def-priori).
...
Als letztes bildet sich der Golem eine abschließende Meinung zur Wahrscheinlichkeit jeder Hypothese. Diese Wahrscheinlichkeitsverteilung nennt man *Posteriori-Verteilung*, s. @def-post1.
...
:::{#def-priori}
### Priori-Verteilung
Für jede Hypothese haben wir ein Vorab-Wissen, das die jeweilige Plausibilität der Hypothese angibt: *Priori-Verteilung* (synonym: Apriori-Verteilung).$\square$
:::
...
Lassen Sie uns im Folgenden die Wahrscheinlichkeit ($Pr$), $W$ mal Wasser und $L$ mal Land zu beobachten, wenn die Wahrscheinlichkeit für Wasser $\pi$ beträgt, so bezeichnen: $Pr(W,L | \pi))$ oder auch (synonym) so: $Pr(W|\pi, n)$.
Diese Wahrscheinlichkeit, $Pr(W,L | \pi)$, kann man im Fall des Globusversuchs mit der *Binomialverteilung* berechnen.

Möchte man die Wahrscheinlichkeit ansprechen für das Ereignis "6 mal Wasser und 3 mal Land, 
wenn wir von einem Wasseranteil von 70% ausgehen", so würden wir kurz schreiben: $Pr(W=6, L=3 | \pi=.7)$.
Oder man könnte (synonym) schreiben:  $Pr(W=6 | \pi=.7, n=9)$.


Zur Erinnerung: Die Binomialverteilung zeigt die Verteilung der
Wahrscheinlichkeit der Ereignisse (z.B. 2 Mal Kopf) beim wiederholten Münzwurf
(und allen vergleichbaren Zufallsexperimenten): 
"Münzwurfverteilung", s. Kap. @sec-bin-distrib.
...
Was ist der Anteil der gültigen Pfade in einem Baumdiagramm (d.h. die Wahrscheinlichkeit), 
...
#| lst-cap: "Binomialverteilung mit R für x=2, n=3, p=1/2"
...
%%| echo: false
%%| label: fig-binom1a
%%| fig-cap: "Wir werfen den Globus (oder eine Münze) 3 Mal. Die Knoten sind der Übersicht halber mit fortlaufenden Buchstaben (von A bis P) bezeichnet"
flowchart TD
  A[A - Start] -. 1/2 .-> B[B - 0]
  A -. 1/2 .-> C[C - 1]
  B -. 1/2 .-> D[D - 0]
...
### Globus mit 6 Treffern bei 9 Würfen, p=1/2
...
### Globus mit 9 Treffern bei 9 Würfen, p=1/2
...
### Globus mit 6 Treffern bei 9 Würfen, p=70%

Was ist die Wahrscheinlichkeit für $W=6$,
gegeben $n=9$ und $p=.7$?
...
3. Die *Posteriori-Verteilung* (die Wahrscheinlichkeit der Hypothese nach den Daten), s. @fig-post1
...
dass wir von 9 Würfen ausgehen und eine Wahrscheinlichkeit für Wasser von 70% annehmen.


Die Verwendung der Binomialvertielung ist an einige Annahmen geknüpft:

1.  Die Züge sind unabhängig voneinander (Die Würfe des Globusballs beeinflussen sich einander nicht).
2.  Der Parameterwert $\pi$ bleibt konstant (Der Wasseranteil der Erde ändert sich nicht während des Versuchs).
...
Die Evidenz berechnet sich als Summe der Likelihoods über alle Ausprägungen von $D$, d.h. als die totale Wahrscheinlichkeit von $D$, s. @eq-evidenz, vgl. auch @def-totwskt:

$$Pr(D) = Pr(D|H) \cdot Pr(H) + Pr(D| \neg H) \cdot Pr(\neg H)$${#eq-evidenz}


Die Aufgabe der Evidenz ist nur dafür zu sorgen, dass der Bruch insgesamt nur Werte zwischen 0 und 1 annehmen kann. $\square$
:::
...
<!-- Erinnerung wir uns (@exm-kalt-regen): Die Wahrscheinlichkeit für *Regen* und *kalt* ist gleich der Wahrscheinlichkeit von *Regen*, *gegeben kalt* mal der Wahrscheinlichkeit von *kalt*; das ist die Kettenregel (@def-kettenregel). $\square$ -->
<!-- ::: -->
...
Schauen wir uns die Bestandteile von Bayes' Theorem (@eq-bayes1) noch etwas näher an:
...
mit der $Pr_{Priori}$ und dem $L$ füttert.
...
Wir erstellen uns eine kleine Tabelle, die man "Bayes-Box" nennen könnte.^[Auch Gitter-Methode oder Grid-Methode genannt.]
Dazu gehen wir so vor:

### Die Idee der Bayes-Box

1.  Teile den Wertebereich des Parameters in ein "Gitter" auf, z.B. $0.1, 0.2, ..., 0.9, 1$.
2.  Wähle den Priori-Wert des Parameters für jeden Parameterwert, z.B. 1/11 bei einer Gleichverteilung von 0 bis 1.
...
Dazu geben wir an, für wie wahrscheinlich wie apriori^[synonym: priori] 
...
(da wir ein binäres Ereignis, $W$ oder $L$, haben).
Der Likelihood gibt an, 
wie wahrscheinlich ein Parameterwert ist gegeben einem bestimmten apriori gewählten Parameterwert.
...
Schließlich packen wir das alles in eine Tabelle, die "Bayes-Box", s. @tbl-globus und @lst-gitter1.
...
#| label: tbl-globus
#| tbl-cap: "Die Bayes-Box für den Globusversuch, k=6 Treffer, n=9 Versuche, Apriori-Wahrscheinlichkeit Pr(H)=9%, und Wasseranteile p von 0 bis 1"
#| echo: false
d %>% 
  mutate(id = 1:11) %>% 
...
Dabei haben wir die Priori-Wahrscheinlihkeit für alle Parameterwerte als gleich angenommen, da wir keinerlei Vorwissen hatten, $Pr(H_i) = 1/11$.
...
In @exm-globus697 haben wir die Wahrscheinlichkeit für 6 Treffer bei 9 Würfen gegeben einer Trefferwahrscheinlichkeit von $\pi = .7$ berechnet.
Damit haben wir die Likelihood $L = Pr(D|H) =.25$ berechnet.


Auf dieser Basis können wir die Posteriori-Wahrscheinlichkeit $Pr_{Post}$ berechnen, zunächst die unstandardisierte.
...
hat sich unsere Meinung über den Wasseranteil geupdatet von 0.1 auf 0.25.$\square$
:::
...
wie wahrscheinlich Ihre Hypothese (Ihr Lieblings-Parameterwert) ist. Und noch einiges mehr, aber das ist Thema des nächsten Kapitels. $\square$
:::
...
:::{#exr-tipp-p}
🏋️ Wenn Sie auf einen Prozentwert für $W$ tippen müssten, welchen würden Sie nehmen, laut dem Modell (und gegeben der Daten)? $\square$
:::
...
1. [Verteilungen-Quiz-01](https://sebastiansauer.github.io/Datenwerk/posts/verteilungen-quiz-01/verteilungen-quiz-01)
2. [globus1](https://sebastiansauer.github.io/Datenwerk/posts/globus1/index.html)
2. [globus2](https://sebastiansauer.github.io/Datenwerk/posts/globus2/index.html)
3. [globus3](https://sebastiansauer.github.io/Datenwerk/posts/globus3/index.html)
4. [globus-bin](https://sebastiansauer.github.io/Datenwerk/posts/globus-bin/index.html)
4. [globus-bin2](https://sebastiansauer.github.io/Datenwerk/posts/globus-bin2/index.html)
3. [Krebs1](https://sebastiansauer.github.io/Datenwerk/posts/krebs1/krebs1)
2. [kekse01](https://sebastiansauer.github.io/Datenwerk/posts/kekse01/kekse01)
2. [kekse03](https://sebastiansauer.github.io/Datenwerk/posts/kekse03/kekse03)
2. [bayes2](https://sebastiansauer.github.io/Datenwerk/posts/bayes2/bayes2)
3. [Bayes-Theorem1](https://sebastiansauer.github.io/Datenwerk/posts/bayes-theorem1/bayes-theorem1) 
5. [bayes-ziel1](https://sebastiansauer.github.io/Datenwerk/posts/bayes-ziel1/bayes-ziel1)
6. [totale-wskt1](https://sebastiansauer.github.io/Datenwerk/posts/totale-wskt1/totale-wskt1.html)
7. [wskt-quiz13](https://sebastiansauer.github.io/Datenwerk/posts/wskt-quiz13/wskt-quiz13)
8. [wskt-quiz12](https://sebastiansauer.github.io/Datenwerk/posts/wskt-quiz12/wskt-quiz12)
8. [wskt-quiz15](https://sebastiansauer.github.io/Datenwerk/posts/wskt-quiz15/wskt-quiz15)


### Aufgaben, für die man einen Computer braucht

<!-- 2. [Rethink2E4](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2e4/Rethink2e4) -->
2. [Rethink2m1](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m1/Rethink2m1.html)
2. [Rethink2m2](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m2/Rethink2m2)
2. [Rethink2m3](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m3/Rethink2m3)
<!-- 2. [Rethink2m4](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m4/Rethink2m4) -->
<!-- 2. [Rethink2m5](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m5/Rethink2m5) -->
<!-- 2. [Rethink2m6](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m6/Rethink2m6) -->
<!-- 2. [Rethink2m7](https://sebastiansauer.github.io/Datenwerk/posts/Rethink2m7/Rethink2m7) -->
2. [kekse02](https://sebastiansauer.github.io/Datenwerk/posts/kekse02/kekse02.html)
2. [euro-bayes](https://sebastiansauer.github.io/Datenwerk/posts/euro-bayes/euro-bayes.html)
3. [bath42](https://sebastiansauer.github.io/Datenwerk/posts/bath42/bath42)
4. [Kaefer2](https://sebastiansauer.github.io/Datenwerk/posts/kaefer2/kaefer2)
5. [rethink3m1](https://sebastiansauer.github.io/Datenwerk/posts/ReThink3m1/ReThink3m1.html)
1. [Lose-Nieten-Binomial-Grid](https://sebastiansauer.github.io/Datenwerk/posts/lose-nieten-binomial-grid/lose-nieten-binomial-grid)
