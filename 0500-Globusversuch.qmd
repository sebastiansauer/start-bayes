...
$Pr(H| D)$ kann man  umformen (vgl. @eq-pr-cond),
...
-- also bevor wir irgendwelche Daten erheben -- jeden einzelnen Gitterwert halten.
...
Konkret gesprochen: Wir listen jeden möglichen Wasseranteil ($\pi$) auf,
also $\pi=0, 0.1, 0.2, ..., 1$.
...
#| lst-cap: "Gitter für Wasseranteile: 0, 0.1, 0.2, ..., 1"
...
#| label: tbl-globus
#| tbl-cap: "Die Bayes-Box für den Globusversuch, k=6 Treffer, n=9 Versuche, Apriori-Wahrscheinlichkeit Pr(H)=0.1, und Wasseranteile p von 0 bis 1"
#| echo: false
d %>% 
  mutate(id = 1:11) %>% 
...
der unstandardisierten Post-Wahrscheinlichkeiten, $Pr(D) = 0.1$.
