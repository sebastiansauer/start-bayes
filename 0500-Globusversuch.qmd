...
Man muss sich bei der Interpretation eines Regressionsmodell vor Augen halten: "Die Ergebnisse des Modell sind nur richtig unter der Annahme, dass sich der Zusammenhang X und Y durch eine Gerade beschreiben lasssen und unter der Annahme, dass meine Daten repräsentativ sind." $\square$
:::
...
Sie berechnet als Gewichtung des Vorwissen mit den neuen Daten, die durch die Likelihood repräsentiert werden.
...
Wir haben den Globus 9 Mal geworfen und 6 Mal Wasser beobachtet.
Das sind unsere Daten.
Jetzt interessiert uns:
Welche Hypothese (Welcher Wasseranteil, $\pi$) ist am plausibelsten,
gegeben diese Daten?


Wie wahrscheinlich ist ein bestimmter Wasseranteil, z.B. 90%, auf der Erde,
wenn wir 6 Treffer (bei 9 Würfen) erhalten haben?
Diese Wahrscheinlichkeit nennt man die *Likelihood*, $L = Pr(\pi=.9| x = 6, n = 9)$.
...
Berechnen wir also die Likelihood für verschiedene Hypothesen (Wasseranteile, $\pi$), für $\pi = 0, 0.1, 0.2, \ldots, 1$,
s. @lst-lik1.

```{r lik1}
#| lst-cap: "Likelihood-Funktion für x=6 Treffer, bei n=9 Würfen, für eine binomialverteilte Zufallsvariable"
#| lst-label: lst-lik1
#| results: hold 
...
@fig-lik1 visualisiert die Likelihood-Funktion für unser Beispiel (6 Wasser bei 9 Würfen) für verschiedene Hypothesen (Wasseranteile, $\pi$).

```{r}
#| fig-cap: "Likelihood-Funktion für x=6 Treffer, bei n=9 Würfen, für eine binomialverteilte Zufallsvariable"
#| label: fig-lik1
#| echo: false
...
### Likelihood für $\pi=6/9$
Was ist die Likelihood von $p=.7$, gegeben $W=6$ bei $n=9$ bei unserem Globusversuch?
...
$\pi=.7$ ist offenbar eine gute Hypothese, um die Daten zu erklären. $\square$
:::
...
### Likelihood für $\pi=1/3$
Was ist die Likelihood von $p=1/3$, gegeben $W=6$ bei $n=9$ bei unserem Globusversuch?

```{r QM2-Thema2-kleineModelle-22a, echo = TRUE}
dbinom(x = 6, size = 9, prob = 1/3)
```

Offenbar ist $\pi=1/3$ eine schlechte Hypothese, um die Daten zu erklären. $\square$
...
Was ist die Likelihood von $\pi=0$, gegeben $W=6$ bei $n=9$ bei unserem Globusversuch?
...
$\pi=0$ ist offenbar eine sehr schlechte Hypothese, um die Daten zu erklären. $\square$
...
Wir führen wieder den Globusversuch durch (oder werfen eine Münze) und erhalten folgendes Ergebnis: 6 Mal Wasser und 3 Mal Land (also $W=6$ und $L=3$).

Welcher Wasseranteil ist am plausibelsten?
...
Die Aufgabe der Evidenz ist nur dafür zu sorgen, dass der Wert von $Pr(H|D)$ insgesamt nur Werte zwischen 0 und 1 annehmen kann, 
...
<!-- $$Pr(H|D) = \frac{Pr(D|H) \cdot Pr(H)}{Pr(D)} = \frac{\text{Likelihood}  \cdot \text{Apriori-Wahrscheinlichkeit}}{\text{Evidenz}}$${#thm-bayes5} -->
...
Abb. @fig-post3 visualisiert, dass die Post-Verteilung eine Gewichtung von Apriori und Likelihood ist (das gilt sowohl für die unstandardisierte als auch für die standardisierte Post-Verteilung).
...
2. Wähle den Apriori-Wert des Parameters für jeden Parameterwert, z.B. 1/11 bei einer Gleichverteilung von 0 bis 1.
...
5. Standardisiere den Aposteriori-Wert durch teilen anhand der Summe alle unstand. Aposteriori-Werte.
...
Dabei haben wir die Apriori-Wahrscheinlichkeit für alle Parameterwerte als gleich angenommen, da wir keinerlei Vorwissen hatten, $Pr(H_i) = 1/11$.
...
hat sich unsere Meinung über den Wasseranteil von $\pi=.7$ aktualisiert von 0.09 auf 0.27 - das ist eine Verdreifachung. 
Wir sind uns drei Mal so sicher wie vor dem Versucht, dass der Wasseranteil bin 70% liegt. $\square$
:::
