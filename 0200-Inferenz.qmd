...
In der *Bayes-Statistik* ist der Begriff nicht einheitlich definiert. Mit Bezug auf @gelman2021 wird in diesem Buch der Begriff wie folgt definiert.


:::{#def-signifikanz}
### Statistische Signifikanz
Ist Null nicht im Konfidenzintervall enthalten, so liegt ein statistisch signifikantes Ergebnis vor. $\square$
:::

Oft werden 95%-Konfidenzintervalle verwendet, obwohl das nur eine Konvention ist. Die Signifikanzaussage bezieht sich immer auf die Größe des Konfidenzintervalls (und des Modells inklusive Daten).

Liegt ein statistisch signifikantes Ergebnis vor, so verwirft man die Nullhypothese.

:::{#def-nullhyp}
### Nullhypothese
Die Nullhypothese, $H_0$, besagt, dass kein Effekt (Unterschied, Zusammenhang, Veränderung, ...) vorliegt. $\square$
:::
...
Eine Möglichkeit ist Kennwert *pd* (*probability of direction*).
