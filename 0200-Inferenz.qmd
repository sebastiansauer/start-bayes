...
Abb. @fig-goals gibt einen Überblick über die Ziele der Statistik.
...
Allerdings hülfe es, wenn man andere Menschen vom Nutzen der eigenen Ideen überzeugen kann.
:::
...
Statistische Inferenz sieht sich drei "Herausforderungen" gegenüber, laut  @gelman_regression_2021, Kap. 1.1. 
...
Nehmen wir an, wir möchten herausfinden, wie groß der Anteil der R-Fans an der Population der Studierenden ist. Den Anteil der F-Fans bezeichnen wir der Einfachheit halber hier mit `A`^[~~Meistens~~ Manchmal darf man bei der Statistik nicht nach einem tieferen Sinn suchen. Ist Statistik eine Art moderne Kunst?].
...
Wir müssen also den Anteil der R-Fans auf Basis des Anteils in der Stichprobe für die Grundgesamtheit schließen:
...
Ah, in unserer Stichprobe sind 42% R-Fans!^[Mancheiner hätte mit mehr gerechnet]. Man schreibt: $p = 0.42$ (`p` wie `proportion`). Die Stichprobe sei repräsentativ für die Grundgesamtheit aller Studierender. 
...
Sie besitzen keine ontologische Realität, sondern eine epistemologische.
Sie sind frei, sich andere Einteilungen der Statistik auszudenken.
Es hilft allerdings, wenn man andere Menschen vom Wert seiner Idee überzeugen kann.
...
-   Wie groß ist die Wahrscheinlichkeit[^inferenz-1] $Pr(\mu_{V1} > \mu_{V2})$?

[^inferenz-1]: oft mit *Pr* oder *p* abgekürzt, für *probability*
...
Was ist das "Wesentliche"? Meist interessiert man sich für die Ursachen eines Phänomens? Etwa: "Wie kommt es bloß, dass ich ohne zu lernen die Klausur so gut bestanden habe?"^[Das ist natürlich nur ein fiktives, komplett unrealistisches Beispiel, das auch unklaren Ursachen den Weg auf diese Seite gefunden hat.]
Noch allgemeiner ist vom häufig am Zusammenhang von `X ` und `Y` interessiert, s. @fig-xy, linker Teil, die ein Sinnbild eines statistischen Modells widergibt.
...
Okay, ein statistisches Modell kann natürlich komplexer sein, z.B. wie in Abb. @fig-xy, rechter Teil, dargestellt.
...
![Wähle deine Statistik mit Bedacht](img/choose-test.png){#fig-choose-test}

### Viele statistische Verfahren sind Spezialfälle der Regression

Wie Jonas Kristoffer Lindeløv uns erklärt, sind viele statistische Verfahren, wie der sog. t-Test Spezialfälle der Regression, s. Abb. @fig-lindeloev.
...
Dabei wird X nicht mit "fortgeschrittenen" Transformationen wie Quadradieren oder Exponenzieren beglückt, sondern nur mit den Regressiongewichten multipliziert.
...
Schließt man etwa, dass in einer Grundgesamtheit der Anteil der R-Fans bei 42% liegt, so geschieht das unter Unsicherheit.
...
1. Wie (un)gewiss ist man sich über den Wert des Regressionsgewichts?

2. Wie (un)gewiss ist man sich über den Wert von Y? Schließlich könnte es ja Einflüsse (X) geben, die man nicht berücksichtigt hat.
...
Streng genommen ist eine Inferenz aus Angabe der Ungewissheit (Genuaigkeit der Schätzung) wertlos.
Angenommen, jemand sagt, dass sie den Anteil der R-Fans (in der Population) auf 42% schätzt, lässt aber offen wie *sicher* (präzise) die Schätzung ist.
...
:::callout-important
Eine Inferenz nennt man auch Schätzung. 
Es sollte immer die Genauigkeit (Ungewissheit) der Schätzung angegeben werden.
:::
...
2.  zu Einflüssen (X), die unser Modell nicht kennt ($\epsilon, \sigma$)
...
![Eine Punktschätzung und ihre Ungewissheit](img/fig-punktschaetzer.png){#fig-punktschaetzer2}
...
In Abb. @fig-punktschaetzer2, rechts, ist die Ungewissheit in den Regressionskoeffizienten visualisiert: Wie sicher sind wir uns zur Stärke des Zusammenhangs von X und Y, vgl. @def-punktschaetzer.



Auch wenn wir uns *sicher* im Hinblick auf die Regressionsgewichte in Abb. @fig-ungewiss2 *bliebe eine Restungewissheit*:
Unsere Schätzungen wären auch dann nicht sicher, nicht fehlerfrei.
Das liegt daran, da das Modell nicht alle Einflüsse auf Y berücksichtigt, sondern nur einen, hier als X bezeichnet.
...
![Das Vorhersageintevall zeigt eine Punktschätzung und ihre Ungewissheit](img/fig-predintervall.png)
...
Ein Konfidenzintervall wird häufig mit 90% oder 95% Genauigkeit angegeben.  Im Kontext der Bayes-Analyse ist das einfach zu interpretieren. 
...
Dieser Befund lässt sich so interpretieren: "Laut Modell liegt der gesuchte Anteil mit einer Wahrscheinlichkeit von 95% im Bereich von 44 bis 44 Prozentpunkten."
...
[In diesem Post](https://data-se.netlify.app/2022/01/27/warum-bayes/) wird für Bayes geworben und (vielleicht einseitig) Stellung pro Bayes bezogen.
...
Eine Studie (vgl. @gelman_regression_2021) fand einen "großen Effekt" auf das Einkommen von Babies, eine Stunde pro Woche während zwei Jahren an einem psychosozialen Entwicklungsprogramm teilnahmen (im Vergleich zu einer Kontrollgruppe), $n=127$.
