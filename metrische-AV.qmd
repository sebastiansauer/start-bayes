...
if (knitr:::is_html_output()) {
  icon_style(fontawesome("angry", style = "solid"), scale = 3, fill = "red")
}
```
...
#| echo: false
if (knitr:::is_html_output()) {
  icon_style(fontawesome("tired", style = "solid"), scale = 3, fill = "red")
}
```
...
Und die Visualisierung des Modells `m10.3`, s. @fig-m103.^[Alternativ kann man die gleiche Visualisierung - einfacher - so bekommen: `plot(estimate_expectation(m10.3))`. Allerdings müssen Sie dann, bevor Sie m10.3 berechnen, `mom_hs` als Faktorvariable definiernen mit `mutate` und `as.factor(mom_hs)`.]

```{r Thema6-Teil2-5, fig.asp = 0.62}
#| label: fig-m103
#| fig-cap: "Der Effekt von sowohl mütterlicher Intelligenz als auch mütterlichem Schulabschluss. Es ist gut zu erkennen, dass der Achsenabschnitt für die Daten nicht interpretierbar ist."
kidiq %>% 
...
@tbl-m105 zeigt die Punktschätzer der Parameter für m10.5 sowie ihre Perzentilintervalle^[auch ETI genannt].
Nutzen Sie dafür `parameters(m10.5)`.

```{r}
#| fig-cap: "Parameter von m10.5 und ETIs"
#| echo: false
#| label: tbl-m10a
parameters(m10.5) %>% 
  display()
...
Für kausale Aussagen ist mehr nötig, als einen Zusammenhang festzustellen.
:::
...
```{r discribe-kidiq}
#| echo: false
describe_distribution(kidiq, type = "mean_sd") %>% 
  display() 
```
...
Mit `parameters` können wir uns ein PI für `m10.10` ausgeben lassen, s. @fig-m1010hdi;
 im Standard wird ein ETI berichtet.
...
```{r resid-lm-plot-mtcars, fig.asp = .619}
#| echo: false
