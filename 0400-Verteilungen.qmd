...
Lesen Sie selbständig, zusätzlich  zum Stoff dieses Kapitels, noch @bourier2011; dort folgende Abschnitte:
...
Dabei interessiert die Anzahl der $k$ Treffer.
...
Außerdem hat ein bestimmtes Ergebnis im ersten Wurf keinen Einfluss auf die Wahrscheinlichkeit eines bestimmten Ergebnisses im zweiten Wurf, etc.^[sog. "iid": *independent and identically distributed*.]$\square$ 
:::
...
Stellen wir uns eine Kistchen^[In den Lehrbüchern häufig als Urne bezeichnet, was den bösen Spott von "Friedhofstatistik" nach sich zog.] mit $n=5$ Losen vor, 
darunter $k=2$ *T*reffer (Gewinn) und $5-2=3$ *N*ieten, s. Abb. @fig-urne.
...
Um die Wahrscheinlichkeitsverteilung einer binomialverteilte Zufallsvariable ausrechnen zu können, muss man zwei Dinge wissen: Erstens die Anzahl der Münzwürfe, $n$ (Stichprobengröße) und zweitens die Trefferwahrscheinlichkeit, $p$. Mit diesen Informationen kann man z.B. ausrechnen, wie groß die Wahrscheinlichkeit ist, bei 5 Würfen 2 Treffer zu erzielen.

:::


:::{#exr-bin1}
#### Fünf Lose, zwei Treffer
Wie groß ist die Wahrscheinlichkeit bei $n=5$ Zügen $k=2$ Treffer zu erzielen (und $n-k=3$ Nieten)? (Nennen wir dieses Ereignis der Kürze halber $A^{\prime}$ ).
Im ersten Zug ist die Trefferwahrscheinlichkeit $p=2/5$ und die 
Nietenwahrscheinlichkeit $1-p=3/5$.
Wir ziehen dabei *mit* Zurücklegen (ZmZ). 
Außerdem sind die Lose nicht zu unterscheiden (abgesehen davon, ob es Treffer oder Nieten sind).$\square$
:::


Mit Blick auf @exr-bin1: 
Wir könnten jetzt ein Baumdiagramm zeichnen und pro Pfad die Wahrscheinlichkeit ausrechnen
(*Multiplikationssatz*, @eq-multtheorem), vgl. @fig-dreimuenzen.
...
$Pr(A) = Pr(T)^2 \cdot Pr(N)^2 = \left( \frac{2}{5} \right)^2 \cdot \left( \frac{3}{5} \right)^3$.
...
Etwas mühevolles Zählen der Pfade würde uns zeigen, dass es $k=10$ Pfade gibt, die alle die gleiche Wahrscheinlichkeit, $Pr(A)$, aufweisen.
Wir könnten auch R auffordern, den Binomialkoeffizienten zu verwenden, um die Anzahl der Pfade zu berechnen, s. @eq-binkoeff:
...
Damit beträgt die Wahrscheinlichkeit des gesuchten Ereignisses $A^{\prime}$ (2 Treffer bei 5 Zügen):

$Pr(A^{\prime}) = 10 \cdot Pr(A)$.
...
Die Wahrscheinlichkeit, bei 5 Zügen 2 Treffer zu erzielen mit $p=2/5$ (unter der Annahme einer Binomialverteilung) lässt sich so mit R berechnen:

```{r}
dbinom(x = 2, size = 5, prob = 2/5)
```
...
::: {.column width="50%"}
>    👨‍🏫 Hey R, Wie viele Möglichkeiten gibt es, aus $n=5$ Losen $k=2$ auszuwählen?
...
:::{#exm-2aus4}
Hier ist ein Überblick der möglichen 6 günstigen Ereignisse des Experiments (2 Treffer bei 4 Versuchen):
...
TTNNN, TNTNN, TNNTN, TNNNT, NTTNN, NTNTN, NTNNT, NNTTN, NNTNT, NNNTT $\square$
:::
