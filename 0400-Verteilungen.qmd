...
Der Stoff dieses Kapitels deckt sich (weitgehend) mit @bourier2011, Kap. 6.1 und 6.3 sowie 7.1 und 7.2.



Dieses Kapitel setzt einige Grundbegriffe voraus,
wie im Buch [Statistik1](https://sebastiansauer.github.io/statistik1/)^[<https://sebastiansauer.github.io/statistik1/>] vorgestellt, insbesondere im Kapitel ["Rahmen"](https://sebastiansauer.github.io/statistik1/010-rahmen#was-sind-daten)^[<https://sebastiansauer.github.io/statistik1/010-rahmen#was-sind-daten>].
...
::: {.callout-note}
Die Folien zu diesem Kapitel finden Sie [hier](slides/0400-Verteilungen.html).
:::
::::
...
Wenn man keinen hinreichenden Grund hat, eine bestimmte Ausprägung einer Zufallsvariablen für plausibler als eine andere zu halten,
...
@fig-uniform, (a):
Bei $X=0$ hat *eine Einheit* von $X$ (z. B. von -1 bis 0) die Wahrscheinlichkeitsmasse von 50 %,
...
@fig-uniform, (b):
Bei $X=0$ hat *eine Einheit* von $X$ die Wahrscheinlichkeitsmasse von ca. 33 %,
...
Die Binomialverteilung dient zur Darstellung der Wahrscheinlichkeit der Ergebnisse eines $n$-fach wiederholten binomialen Zufallsexperiments,
...
Bei jeder Wiederholung liegt die Wahrscheinlichkeit eines Treffers bei $p$, und diese Wahrscheinlichkeit bleibt über alle Wiederholungen hinweg gleich:
Die Münze verändert sich nicht durch die Würfe (*Ziehen mit Zurücklegen*, ZmZ).
Außerdem hat ein bestimmtes Ergebnis im ersten Wurf keinen Einfluss auf die Wahrscheinlichkeit 
eines bestimmten Ergebnisses im zweiten Wurf, etc., sog.
"iid":
...
:::{#exm-binom1}
Anwendungsbeispiele:
Wie viele defekte Teile sind in einer Stichprobe von produzierten Schrauben zu erwarten?
Wie wahrscheinlich ist es, dass das neue Blutdruck-Medikament einer bestimmten Anzahl von Menschen hilft?
Wie viele Personen stimmen in einer Umfrage der Frage "Ich halte die öffentlich-rechtlichen Sender für wichtig." zu?
Was ist die Wahrscheinlichkeit, 6 mal hintereinander eine *6* zu würfeln bei einem fairen Würfel?
Eine Münze, die in 7 von 10 Würfen "Kopf" zeigt -- sollte sie als "unfair" eingeschätzt werden?
$\square$
:::

Stellen wir uns ein Kistchen^[In den Lehrbüchern häufig als Urne bezeichnet, was den bösen Spott von "Friedhofstatistik" nach sich zog.] mit sehr vielen^[praktisch unendlich vielen] Losen vor,
darunter 2/5 *T*reffer (Gewinn) und 3/5 *N*ieten, s. Abb. @fig-urne.
Der Versuch läuft so ab:
Wir ziehen ein Los, schauen ob es ein Treffer ist oder nicht, legen es zurück und ziehen erneut.
Da *sehr* viele Lose im Kästchen liegen, ist es praktisch egal, ob wir das Los wieder zurücklegen.
Die Wahrscheinlichkeit für einen Treffer ändert sich (so gut wie) nicht.
...
```{r echo = FALSE}
#| fig-cap: Ein Kästchen mit Losen, davon 2/5 Treffer und 3/5 Nieten
#| label: fig-urne
#| fig-asp: 0.3
#| echo: false  
...
:
Alle acht Ergebnisse des Zufallsexperiment (binär, 3-fach wiederholt) mit ihren Wahrscheinlichkeiten im Überblick {#tbl-3lose2treffer}
...
(*Multiplikationssatz*, @thm-multtheorem), vgl. @fig-dreimuenzen.
...
so zählt man 3 günstige Pfade (T:
Treffer;
N:
Niete), s. @fig-dreimuenzen:
...
Dabei liegt immer ein Zufallsversuch mit $n$ Durchgängen und $k$ Treffern zugrunde.
Jeder Durchgang hat die Trefferwahrscheinlichkeit $p$ und jeder Durchgang ist unabhängig von allen anderen.
...
@thm-binomial kann man wie folgt auf Deutsch übersetzen:

>   Die Wahrscheinlichkeit für das Ereignis $X$ gegeben $p$ und $n$ berechnet sich als Produkt von zwei Termen.
> Der erste Term gibt die Anzahl der (günstigen) Pfade, $k$ (aus $n$ möglichen Pfaden).
> Der zweite Term gibt die Wahrscheinlichkeit für einen günstigen Pfad (einen Treffer), $p=P(A)$, die "Pfad-Wskt".




Die Anzahl der (günstigen) Pfade kann man mit dem *Binomialkoeffizient* ausrechnen, 
den man mit $\binom{n}{k}$ darstellt, s. @thm-binkoeff.^[wobei gelten muss $n \ge k$]
...
Lies:
"Wähle aus $n$ möglichen Ereignissen (Pfade im Baum) $k$ günstige
Ereignisse (Pfade) oder kürzer "k aus n" bzw. "n über k". $\square$
:::
...
wenn man sich diese lange Übersetzung der Formel in Prosa durchliest.
...
:::{#exm-binkoeff}
### 2 von 4 Freunden auswählen

Sie sind auf einer Party.
Großartige Feier.
Es wird spät, und langsam wird es Zeit,
nach Hause zu fahren.
Heute sind Sie der Fahrer.
In Ihrem Auto haben Sie noch 2 freie Plätze.
Aber 4 Ihrer Freunde möchten bei Ihnen mitfahren.
...
Für den 1. freien Platz stehen alle 4 Freunde zur Auswahl.
Für den 2. freien Platz stehen noch 3 Freunde zur Auswahl.
Das sind 12 Möglichkeiten.


@fig-freunde-auswahl zeigt als Baumdiagramm Ihre Auswahl-Möglichkeiten.
Das Diagramm zeigt auch, welche Möglichkeiten Sie hätten,
wenn Sie alle 4 Freunde auswählen könnten. 




```{dot}
//| label: fig-freunde-auswahl
//| fig-width: 4.5
//| fig-height: 4.5
//| fig-cap: |
//|   Sie wählen nacheinander 4 Freunde (A,B,C,D) aus. 
//|   Wie viele Möglichkeiten gibt es dafür?
//|   Zuerst haben Sie 4 Möglichkeiten.
//|   Nachdem Sie den 1. Freund gewählt haben, haben Sie noch 3 weitere zur Auswahl.
//|   Dann noch 2 und schließlich bleibt der Letzte.
digraph permutations4 {
    layout=twopi;
    overlap=false;
    ...
    /* Ebene 4 (Endknoten – 24 Permutationen) */
    ABC -> ABCD;
    ABD -> ABDC;
...
Zum Beispiel "ABCD", "ABDC", "ADBC" etc.
Wir wählen aber nur 2 von 4 Freunden.
Daher haben wir nur $4\cdot 3 \cancel{\cdot 2 \cdot 1} = \frac{4!}{2!} = 12$ Möglichkeiten.
Zum Beispiel "AB", "BA", "AC".
Allerdings ist es für die Frage, wer in Ihrem Auto mitfährt, egal,
ob Sie "AB" oder "BA" auswählen -- das läuft auf die gleichen Mitfahrer heraus.
Also müssen wir diese Variationen noch von unseren Möglichkeiten abziehen.
Wie viele Variationen für die Mitfahrer "A" und "B" gibt es?
Das sind 2 Möglichkeiten:
Für den 1. freien Sitz haben Sie die Wahl zwischen 2 Personen,
für den 2. freien Sitz dann nur noch eine Wahl.
Das macht also $2!$ Variationen -- davon behalten wir nur eine.
Wir "schneiden" also jeden 2.
Baumast ab, teilen also die Anzahl der Äste (Pfade)
durch 2.

Insgesamt gibt es also 6 mögliche Mitfahrer-Kombinationen:
"AB", "AC", "AD", "BC", "BD", "CD".
...
Eine Professorin stellt eine Klausur mit 20 Richtig-Falsch-Fragen.
Wie groß ist die Wahrscheinlichkeit,
durch bloßes Münzwerfen *genau* 15 Fragen richtig zu raten?^[Hey,
endlich mal was für das echte Leben!]
...
Praktischerweise gibt es einen R-Befehl, der das Aufsummieren für uns übernimmt:
`pbinom`.
...
Die Wahrscheinlichkeit, höchstens 15 Treffer (d. h. 0, 1, 2, ...
15 Treffer) zu erzielen, liegt für `prob = .5` laut Binomialverteilung mit `pbinom` bei gut 99 %.
...
Nach dem Multiplikationssatz gilt also:
...
:::{#exr-binom}
️ Was fällt Ihnen bei der Binomialverteilung auf?
Ist sie symmetrisch?
...
Die Binomialverteilung ist in R eingebaut;
man kann sich leicht entsprechende Wahrscheinlichkeiten ausrechnen lassen:
z. B. `dbinom(x = 2, size = 3, prob = 2/5)`.
...
Anders gesagt:
Der Binomialkoeffizient sagt uns,
wie viele Möglichkeiten es gibt, aus z. B. 49 Bällen 6 zu ziehen
...
In R kann man sich die Fakultät mit dem Befehl `factorial` ausrechnen lassen.
Der R-Befehl `choose` berechnet den Binomialkoeffizienten.^[Bei Taschenrechnern ist diese Funktion oft als "nCr" zu finden.]
...
:::{#exm-befördern}
### Beförderung
Aus einem Team mit 25 Personen sollen 11 Personen befördert werden.
Wie viele mögliche Kombinationen (von beförderten Personen) können gebildet werden?
...
Es gibt 4457400 Kombinationen von Teams;
dabei ist die Reihenfolge der Ziehung nicht berücksichtigt. $\square$
:::
...
Mit einer Wahrscheinlichkeit von 5 % fällt ein Motor aus und ist für den Rest des Tages nicht einsatzbereit.
Der Betrieb kann aufrecht erhalten werden, solange mindestens 5 Motoren arbeiten.
Wie groß ist die Wahrscheinlichkeit, dass die Pumpstation aus dem Betrieb fällt?

$Pr(X=k)$ (oder kurz:
$Pr(k)$) gibt die Wahrscheinlichkeit (Wahrscheinlichkeitsfunktion) an für das Ereignis, dass *k* Motoren arbeiten.
...
Es gilt also $Pr(X=5) \approx .04$.
Die Wahrscheinlichkeit, dass (nur) 5 Motoren an einem beliebigen Tag laufen, ist demnach relativ gering -- wobei "gering" subjektiv ist, die Betreiberfirma findet diese Wahrscheinlichkeit,
dass 2 Motoren ausfallen, wohl viel zu hoch.
Die Wahrscheinlichkeit, dass $k=0 \ldots 7$ Motoren laufen, ist in @fig-motoren dargestellt.


`dbinom()` steht für die Wahrscheinlichkeits*d*ichte (im diskreten Fall, wie hier, Wahrscheinlichkeitsfunktion genannt) und `binom` für die Binomialverteilung.
`x` gibt die Anzahl der Treffer an (das gesuchte Ereignis, hier 5 Motoren arbeiten);
`size` gibt die Stichprobengröße an (hier 7 Motoren).
...
Die Wahrscheinlichkeit, dass mind.
5 Motoren arbeiten, beträgt also
`r round(p_mind_5, 4)`.
...
Das sind also `r round(p_weniger_als_4, 4)*100`% Wahrscheinlichkeit, dass die Pumpstation ausfällt.
...
`q = 4` steht für $X \le 4$, also für höchstens 4 Treffer (arbeitende Motoren);
`size = 7` meint die Stichprobengröße, hier 7 Motoren;
`prob` gibt die Trefferwahrscheinlichkeit an.
$\square$
...
E) Es ist unmöglich, dass alle 10 USB-Sticks in Ordnung sind.
$\square$
:::
...
Man "spiegelt" die negative Seite auf die positive.
$\square$
:::


Man kann sie verwenden, wenn man von normalverteilten Werten ausgeht, die größer als Null sein müssen.
Sie hat nur einen Parameter, die Streuung (SD) der zugrundeliegenden Normalverteilung mit Mittelwert 0.
...
- [Schiefe1](https://datenwerk.netlify.app/posts/schiefe1/schiefe1.html)
...
```{r quiz-verteilungen-setup}
#| include: false
library(exams2forms)

quiz_verteilungen_files <- list(
  "exr/gleichverteilung-dichte-schoice/gleichverteilung-dichte-schoice.Rmd",
  "exr/gleichverteilung-eigenschaften-schoice/gleichverteilung-eigenschaften-schoice.Rmd",
  "exr/binomialkoeffizient-ausschuss-schoice/binomialkoeffizient-ausschuss-schoice.Rmd",
  "exr/binomialverteilung-callcenter-schoice/binomialverteilung-callcenter-schoice.Rmd",
  "exr/binomialverteilung-serverfarm-schoice/binomialverteilung-serverfarm-schoice.Rmd",
  "exr/halbe-normalverteilung-eigenschaften-schoice/halbe-normalverteilung-eigenschaften-schoice.Rmd",
  "exr/exponentialverteilung-eigenschaften-schoice/exponentialverteilung-eigenschaften-schoice.Rmd",
  "exr/exponentialverteilung-median-schoice/exponentialverteilung-median-schoice.Rmd",
  "exr/binomialverteilung-symmetrie-schoice/binomialverteilung-symmetrie-schoice.Rmd",
  "exr/binomialverteilung-marketing-schoice/binomialverteilung-marketing-schoice.Rmd"
)
```
...
#| message: false
#| results: asis
exams2forms(quiz_verteilungen_files, n = 1)
```
